...
def exec_init(dev_hash, tool, mode):
    ...
    if target in cache:
        pre_compiled = True
        print(
            "[{}] Skip codogen. Found pre-built in cache".format(
...
def tvm_callback_exec_evaluate(platform, mode, host_only):
    ...
    if platform == "vivado_hls":
        ...
        if not os.path.isfile(os.path.join(Project.path, "kernel.cpp")):
            ...
            replace_text(
                os.path.join(Project.path, "host.cpp"), '#include "kernel.h"', ""
            )
...
def copy_and_compile(platform, mode, backend, host_only, cfg, script):
    ...
    if platform == "rocket":
        ...
        emulator = os.path.join(
            ppac,
            "rocket/emulator/emulator-freechips."
        ...
        if not os.path.isfile(emulator):
            cmd = "cd " + ppac + ";"
            cmd += (
                "cp src/Ppac.v rocket/src/main/resources/vsrc;"
    ...
    # copy sdsoc makefile
        ...
        replace_text(
            os.path.join(Project.path, "utils.h"),
            "xilinx_aws-vu9p-f1-04261818_dynamic_5_0",
            "xilinx_vcu1525_dynamic_5_1",
        )
        if backend == "vhls":
            replace_text(
                os.path.join(Project.path, "Makefile"), "kernel.cl", "kernel.cpp"
            )
        ...
        if mode == "sw_sim":
            env = os.environ.copy()
            assert "AWS_PLATFORM" in os.environ, "aws platform info missing"
            ...
            if False and os.path.isfile("top_function_0.sw_emu.xclbin"):
                ...
                cmd += (
                    "make ocl OCL_TARGET=sw_emu \
                      ...
                      APPLICATION_DIR="
                    + os.path.join(os.getcwd(), Project.path)
                )
        ...
        env = os.environ.copy()
        assert "XDEVICE" in os.environ, "vitis platform info missing"
        ...
        run_process(
            "cp " + os.path.join(Project.path, "kernel.xclbin") + " {}".format(cache)
        )
        return "success"
        ...
        if mode == "sw_sim":
            ...
            if int(ver[0]) < 19:
                raise RuntimeError(
                    "AOC version {}.{}.{} is too old, ".format(*ver)
        ...
        if cfg != "":
            ...
            print(
                "[{}] Running custom commands: {}".format(
