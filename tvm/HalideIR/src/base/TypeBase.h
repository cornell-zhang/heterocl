...
struct halideir_type_t {
   ...
   */
#if __cplusplus >= 201103L
  HALIDEIR_ATTRIBUTE_ALIGN(1)
  halideir_type_code_t code;  // halideir_type_code_t
#else
  ...
  /** The number of fractional bits of precision of a single scalar value of
   * this type. */
  ...
  halideir_type_t(halideir_type_code_t code, uint16_t bits, uint8_t lanes = 1,
                  uint8_t fracs = 0)
   ...
   * instances. */
  halideir_type_t()
      : code((halideir_type_code_t)0), bits(0), lanes(0), fracs(0) {}
...
struct halideir_type_of_helper<float> {
  operator halideir_type_t() {
    return halideir_type_t(halideir_type_float, 32);
  }
};
...
struct halideir_type_of_helper<double> {
  operator halideir_type_t() {
    return halideir_type_t(halideir_type_float, 64);
  }
};
