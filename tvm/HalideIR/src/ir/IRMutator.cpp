...
void IRMutator::visit(const Call *op, const Expr &e) {
  ...
  if (!changed) {
    ...
    expr = Call::make(op->type, op->name, new_args, op->call_type, op->func,
                      op->value_index);
  }
}
...
void IRMutator::visit(const For *op, const Stmt &s) {
  ...
  if (min.same_as(op->min) && extent.same_as(op->extent) &&
    ...
    stmt = For::make(op->loop_var, min, extent, op->for_type, op->device_api,
...
void IRMutator::visit(const Free *op, const Stmt &s) { stmt = s; }
...
void IRMutator::visit(const Realize *op, const Stmt &s) {
  ...
  if (!bounds_changed && body.same_as(op->body) &&
    ...
    stmt = Realize::make(op->func, op->value_index, op->type, new_bounds,
...
void IRMutator::visit(const Prefetch *op, const Stmt &s) {
  ...
  if (!bounds_changed) {
    ...
    stmt = Prefetch::make(op->func, op->value_index, op->type, new_bounds);
  }
}
...
void IRMutator::visit(const StreamExpr *op, const Expr &e) { expr = e; }
...
void IRMutator::visit(const Break *op, const Stmt &s) { stmt = s; }

void IRMutator::visit(const While *op, const Stmt &s) {
  Expr condition = mutate(op->condition);
  Stmt body = mutate(op->body);
...
void IRMutator::visit(const Partition *op, const Stmt &s) { stmt = s; }
...
void IRMutator::visit(const Stencil *op, const Stmt &s) {
  ...
  if (body.same_as(op->body)) {
    ...
    stmt = Stencil::make(op->inputs, op->outputs, body, op->burst_width,
