...
Expr IntImm::make(Type t, int64_t value) {
  ...
  // internal_assert(t.bits() == 8 || t.bits() == 16 || t.bits() == 32 ||
  // t.bits() == 64)
...
Expr Cast::make(Type t, Expr v) {
  ...
  internal_assert(t.lanes() == v.type().lanes())
      << "Cast may not change vector widths\n";
...
Expr Select::make(Expr condition, Expr true_value, Expr false_value) {
  ...
  internal_assert(condition.type().is_bool())
      << "First argument to Select is not a bool: " << condition.type() << "\n";
  internal_assert(false_value.type() == true_value.type())
      << "Select of mismatched types\n";
  internal_assert(condition.type().is_scalar() ||
                  condition.type().lanes() == true_value.type().lanes())
      << "In Select, vector lanes of condition must either be 1, or equal to "
         "vector lanes of arguments\n";
...
Expr Load::make(Type type, VarExpr buffer_var, Expr index, Expr predicate) {
  ...
  internal_assert(type.lanes() == index.type().lanes())
...
Stmt AssertStmt::make(Expr condition, Expr message, Stmt body) {
  ...
  internal_assert(message.type() == Int(32) || message.as<StringImm>())
...
Stmt For::make(VarExpr loop_var, Expr min, Expr extent, ForType for_type,
...
Stmt For::make(VarExpr loop_var, Expr min, Expr extent, ForType for_type,
               DeviceAPI device_api, Stmt body, Array<Expr> annotate_keys,
  ...
  for (size_t i = 0; i < annotate_keys.size(); i++) {
    ...
    internal_assert(annotate_values[i].defined())
        << "Annotate value undefined\n";
  }
...
Stmt Provide::make(FunctionRef func, int value_index, Expr value,
                   Array<Expr> args) {
...
Stmt Allocate::make(VarExpr buffer_var, Type type, Array<Expr> extents,
                    Expr condition, Stmt body, Array<Stmt> attrs, Expr new_expr,
                    std::string free_function, Array<Expr> init_values,
                    bool is_const) {
  for (size_t i = 0; i < extents.size(); i++) {
    internal_assert(extents[i].defined()) << "Allocate of undefined extent\n";
    internal_assert(extents[i].type().is_scalar() == 1)
  ...
  for (size_t i = 0; i < init_values.size(); i++) {
    internal_assert(Internal::is_const(init_values[i]))
        << "Allocate init values must be constatns\n";
  }
    ...
    if (const IntImm *int_size = extents[i].as<IntImm>()) {
      ...
      */
      ...
      if (result > (static_cast<int64_t>(1) << 31) - 1) {
        user_error << "Total size for allocation " << name
...
int32_t Allocate::constant_allocation_size() const {
  return Allocate::constant_allocation_size(extents, buffer_var->name_hint);
}
...
Stmt Realize::make(FunctionRef func, int value_index, Type type, Region bounds,
                   Expr condition, Stmt body, Array<Expr> init_values,
                   bool is_const) {
  for (size_t i = 0; i < bounds.size(); i++) {
    ...
    internal_assert(bounds[i]->min.type().is_scalar())
        << "Realize of vector size\n";
    internal_assert(bounds[i]->extent.type().is_scalar())
        << "Realize of vector size\n";
  }
  ...
  for (size_t i = 0; i < init_values.size(); i++) {
    internal_assert(Internal::is_const(init_values[i]))
        << "Realize init values must be constatns\n";
  }
...
Stmt Prefetch::make(FunctionRef func, int value_index, Type type,
                    Region bounds) {
  for (size_t i = 0; i < bounds.size(); i++) {
...
Stmt IfThenElse::make(Expr condition, Stmt then_case, Stmt else_case) {
  internal_assert(condition.defined() && then_case.defined())
...
Expr Call::make(Type type, std::string name, Array<Expr> args,
                CallType call_type, FunctionRef func, int value_index) {
  for (size_t i = 0; i < args.size(); i++) {
...
Expr Shuffle::make(Array<Expr> vectors, Array<Expr> indices) {
  ...
  int input_lanes = 0;
  for (Expr i : vectors) {
    internal_assert(i.type().element_of() == element_ty)
        << "Shuffle of vectors of mismatched types.\n";
    input_lanes += i.type().lanes();
  }
  for (Expr i : indices) {
    const IntImm *v = i.as<IntImm>();
    internal_assert(v) << "Shuffle vector indices must be constant integer\n";
...
bool Shuffle::is_interleave() const {
  ...
  for (int i = 0; i < (int)vectors.size(); i++) {
    for (int j = 0; j < lanes; j++) {
      if (indices[j * (int)vectors.size() + i].as<IntImm>()->value !=
          i * lanes + j) {
        return false;
      }
    }
...
Expr GetSlice::make(Expr a, Expr index_left, Expr index_right) {
  ...
  internal_assert(index_right.defined())
      << "GetSlice of undefined right index\n";
...
Expr SetSlice::make(Expr a, Expr value, Expr index_left, Expr index_right) {
  ...
  internal_assert(index_right.defined())
      << "SetSlice of undefined right index\n";
...
Stmt KernelDef::make(Array<VarExpr> args, Array<Array<Expr>> arg_shapes,
  ...
  for (size_t i = 0; i < args.size(); i++) {
    ...
    for (size_t j = 0; j < arg_shapes[i].size(); j++) {
      internal_assert(arg_shapes[i][j].defined())
          << "KernelDef of undefined shape\n";
    }
  }
...
Stmt Partition::make(VarExpr buffer_var, int dim, int factor,
                     PartitionType partition_type) {
  internal_assert(dim >= 0)
      << "The dimension of partition must be larger than 0\n";
  internal_assert(factor >= 0)
      << "The factor of partition must be larger than 0\n";
...
Stmt Assert::make(Expr condition, Array<Expr> values, std::string message) {
  ...
  internal_assert(condition.defined())
      << "Assert condition of undefined value\n";
...
bool is_ramp(const Array<Expr> &indices, int stride = 1) {
  for (size_t i = 0; i + 1 < indices.size(); i++) {
    if (indices[i + 1].as<IntImm>()->value !=
        indices[i].as<IntImm>()->value + stride) {
      return false;
    }
  }
  return true;
...
template <>
void StmtNode<KernelDef>::accept(IRVisitor *v, const Stmt &s) const {
  v->visit((const KernelDef *)this, s);
}
template <>
void ExprNode<KernelExpr>::accept(IRVisitor *v, const Expr &e) const {
  v->visit((const KernelExpr *)this, e);
}
template <>
...
void StmtNode<StreamStmt>::accept(IRVisitor *v, const Stmt &s) const {
  v->visit((const StreamStmt *)this, s);
}
template <>
...
Call::ConstString Call::call_cached_indirect_function =
