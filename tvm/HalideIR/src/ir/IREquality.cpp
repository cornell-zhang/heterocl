...
IRComparer::CmpResult IRComparer::compare_expr(const Expr &a, const Expr &b) {
  ...
  // Check the cache - perhaps these exprs have already been compared and found
  // equal.
...
IRComparer::CmpResult IRComparer::compare_node_refs(const NodeRef &a,
                                                    const NodeRef &b) {
...
void IRComparer::visit(const Sub *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const Mul *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const Div *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const Mod *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const Min *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const Max *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const EQ *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const NE *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const LT *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const LE *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const GT *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const GE *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const And *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
void IRComparer::visit(const Or *op, const Expr &e) {
  visit_binary_operator(this, op, expr_);
}
...
void IRComparer::visit(const KernelDef *op, const Stmt &s) {
  ...
  for (size_t i = 0; (result == Equal) && (i < op->arg_shapes.size()); i++) {
    ...
    for (size_t j = 0; (result == Equal) && (j < op->arg_shapes[i].size());
         j++) {
...
void check_equal(Expr a, Expr b) {
  ...
  internal_assert(r == IRComparer::Equal)
      << "Error in ir_equality_test: " << r << " instead of "
      << IRComparer::Equal << " when comparing:\n"
      << a << "\nand\n"
      << b << "\n";
}
...
void check_not_equal(Expr a, Expr b) {
  ...
  internal_assert(r1 != IRComparer::Equal && r1 != IRComparer::Unknown &&
                  flip_result(r1) == r2)
      << "Error in ir_equality_test: " << r1 << " is not the opposite of " << r2
      << " when comparing:\n"
      << a << "\nand\n"
      << b << "\n";
}
