...
/** \file
 * Subtypes for Halide expressions (\ref Halide::Expr) and statements (\ref
...
struct BinaryOpNode : public ExprNode<T> {
  ...
  EXPORT static Expr make(Expr a, Expr b) {
    ...
    internal_assert(a.type() == b.type())
        << "BinaryOp of mismatched types: " << a << " " << b << "\n";
...
struct Mul : public BinaryOpNode<Mul> {
  EXPORT static Expr make(Expr a, Expr b) {
    ...
    if (a.type().fracs() > 0 || b.type().fracs() > 0) {
      Type type = a.type();
      node->type = Type(type.code(), type.bits(), type.lanes(),
                        type.fracs() + b.type().fracs());
    } else {
...
/*!
 * \brief Define certain auxiliary attribute for the body to be a symbolic
 * value. This provide auxiliary information for IR passes that transforms body.
 *
 ...
 * only if it doesn't have updates). In a consumer node, the access is
 * read-only. None of this is actually enforced, the node is purely for
 * informative purposes to help out our analysis during lowering. For every
 * unique ProducerConsumer, there is an associated Realize node with the same
 * name that creates the buffer being read from or written to in the body of the
 * ProducerConsumer.
 */
...
struct Call : public ExprNode<Call> {
  ...
  EXPORT static ConstString debug_to_file, reinterpret, bitwise_and,
      ...
      bitcast, rewrite_buffer, random, lerp, popcount, count_leading_zeros,
      ...
      likely_if_innermost, transpose, serialize, deserialize, register_destructor, div_round_to_zero,
  ...
  EXPORT static Expr make(Type type, std::string name, Array<Expr> args,
                          CallType call_type, FunctionRef func,
                          int value_index);

  static Expr make(Type type, std::string name, Array<Expr> args,
                   CallType call_type) {
  ...
  bool is_intrinsic(ConstString intrin_name) const {
    return ((call_type == Intrinsic || call_type == PureIntrinsic) &&
            name == intrin_name);
  }
...
struct Variable : public ExprNode<Variable> {
  ...
  // They can be added back via passing in binding of Variable to specific
  // values. in the final stage of code generation.
...
struct For : public StmtNode<For> {
  ...
  EXPORT static Stmt make(VarExpr loop_var, Expr min, Expr extent,
                          ForType for_type, DeviceAPI device_api, Stmt body);
  ...
  void VisitAttrs(IR::AttrVisitor *v) final {
    ...
    v->Visit("body", &body), v->Visit("annotate_keys", &annotate_keys);
...
struct Shuffle : public ExprNode<Shuffle> {
  ...
  int slice_stride() const {
    return indices.size() >= 2
               ? static_cast<int>(indices[1].as<IntImm>()->value -
                                  indices[0].as<IntImm>()->value)
               : 1;
  }
  ///@}
...
struct Stencil : public StmtNode<Stencil> {
  ...
  EXPORT static Stmt make(Array<VarExpr> inputs, Array<VarExpr> outputs,
