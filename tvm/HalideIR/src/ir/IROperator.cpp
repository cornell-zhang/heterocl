...
Expr make_const(Type t, uint64_t val) { return make_const_helper(t, val); }

Expr make_const(Type t, double val) { return make_const_helper(t, val); }
...
void check_representable(Type dst, int64_t x) {
  if (dst.is_handle()) {
    user_assert(dst.can_represent(x))
        << "Integer constant " << x << " will be implicitly coerced to type "
        << dst << ", but Halide does not support pointer arithmetic.\n";
  } else {
    user_assert(dst.can_represent(x))
        << "Integer constant " << x << " will be implicitly coerced to type "
        << dst << ", which changes its value to " << make_const(dst, x)
        << ".\n";
  }
}
...
void match_types(Expr &a, Expr &b) {
  ...
  if (a.type().is_scalar() && b.type().is_vector()) {
    ...
    internal_assert(a.type().lanes() == b.type().lanes())
...
void match_types_add_sub(Expr &a, Expr &b) {
  ...
  if ((ta.is_fixed() || ta.is_ufixed()) && (tb.is_fixed() || tb.is_ufixed())) {
    ...
    if ((ta.is_fixed() && tb.is_ufixed()) ||
        (tb.is_fixed() && ta.is_ufixed())) {
      tc_int = tc_int + 1;
      tc_code = Type::Int;
    }
...
// Fast math ops based on those from Syrah (http://github.com/boulos/syrah).
// Thanks, Solomon!

// Factor a float into 2^exponent * reduced, where reduced is between 0.75
// and 1.5
void range_reduce_log(const Expr &input, Expr *reduced, Expr *exponent) {
  ...
  *exponent = old_biased_exponent - new_biased_exponent;
  ...
  Expr blended =
      (int_version & non_exponent_mask) | (new_biased_exponent << 23);
...
Expr halideir_log(const Expr &x_full) {
  ...
  float coeff[] = {0.05111976432738144643f,
  ...
  Expr result =
      evaluate_polynomial(x1, coeff, sizeof(coeff) / sizeof(coeff[0]));
...
Expr halideir_erf(const Expr &x_full) {
  user_assert(x_full.type() == Float(32))
      << "halideir_erf only works for Float(32)";
  ...
  float c1[] = {0.0000818502f, -0.0000026500f, 0.0009353904f, 0.0081960206f,
                0.0430054424f, 0.0703310579f,  1.0f};
  ...
  // expansion of erf.
  float c2[] = {-0.0005553339f, 0.0048937243f,  -0.0266849239f,
                0.1127890132f,  -0.3761207240f, 1.1283789803f};
...
Expr fast_log(const Expr &x) {
  ...
  float coeff[] = {0.07640318789187280912f,
...
Expr fast_exp(const Expr &x_full) {
  user_assert(x_full.type() == Float(32))
      << "fast_exp only works for Float(32)";
  ...
  float coeff[] = {0.01314350012789660196f,
...
Expr stringify(const std::vector<Expr> &args) {
  return Internal::Call::make(type_of<const char *>(),
                              Internal::Call::stringify, args,
...
Expr print(const std::vector<Expr> &args) {
  ...
  // Call halideir_print.
  Expr print_call = Internal::Call::make(
      Int(32), "halideir_print", {combined_string}, Internal::Call::Extern);

  // Return the first argument.
  Expr result = Internal::Call::make(
      args[0].type(), Internal::Call::return_second, {print_call, args[0]},
...
Expr print_when(const Expr &condition, const std::vector<Expr> &args) {
  Expr p = print(args);
  return Internal::Call::make(p.type(), Internal::Call::if_then_else,
                              {condition, p, args[0]},
                              Internal::Call::PureIntrinsic);
}
...
Expr require(const Expr &condition, const std::vector<Expr> &args) {
  ...
  user_assert(condition.type().is_bool())
      << "Require condition must be a boolean type\n";
  user_assert(args.at(0).defined()) << "Require of undefined value\n";

  Expr requirement_failed_error = Internal::Call::make(
      Int(32), "halideir_error_requirement_failed",
      {stringify({condition}), combine_strings(args)}, Internal::Call::Extern);
  ...
  return Internal::Call::make(args[0].type(), Internal::Call::if_then_else,
...
Expr memoize_tag_helper(const Expr &result,
                        const std::vector<Expr> &cache_key_values) {
  std::vector<Expr> args;
  args.push_back(result);
  args.insert(args.end(), cache_key_values.begin(), cache_key_values.end());
  return Internal::Call::make(result.type(), Internal::Call::memoize_expr, args,
                              Internal::Call::PureIntrinsic);
}
