...
bool is_simple_const(const Expr &e) {
  ...
  // Don't consider NaN to be a "simple const", since it doesn't obey equality
  // rules assumed elsewere
...
Expr signed_integer_overflow_error(Type t) {
  ...
  return Call::make(t, Call::signed_integer_overflow, {counter++},
                    Call::Intrinsic);
}

// Make a poison value used when integer div/mod-by-zero is detected during
// constant folding.
Expr indeterminate_expression_error(Type t) {
  // Mark each call with an atomic counter, so that the errors can't
  // cancel against each other.
  static std::atomic<int> counter;
  return Call::make(t, Call::indeterminate_expression, {counter++},
                    Call::Intrinsic);
}
...
//      make a new indeterminate_expression of the proper type, set *expr to it
//      and return true.
...
bool propagate_indeterminate_expression(const Expr &e0, const Expr &e1, Type t,
                                        Expr *expr) {
  return propagate_indeterminate_expression(e0, t, expr) ||
         propagate_indeterminate_expression(e1, t, expr);
}

bool propagate_indeterminate_expression(const Expr &e0, const Expr &e1,
                                        const Expr &e2, Type t, Expr *expr) {
  return propagate_indeterminate_expression(e0, t, expr) ||
         propagate_indeterminate_expression(e1, t, expr) ||
...
class Simplify : public IRMutator {
 public:
  Simplify(bool r, const Scope<Interval> *bi, const Scope<ModulusRemainder> *ai)
    ...
    for (Scope<Interval>::const_iterator iter = bi->cbegin();
         iter != bi->cend(); ++iter) {
      int64_t i_min, i_max;
 ...
 private:
    ...
    if (const int64_t *i = as_const_int(e)) {
      *min_val = *max_val = *i;
      ...
      if (const_int_bounds(mul->a, &min_a, &max_a) &&
        ...
        int64_t t0 = min_a * min_b, t1 = min_a * max_b, t2 = max_a * min_b,
                t3 = max_a * max_b;
      ...
      if (const_int_bounds(div->a, &min_a, &max_a) &&
          ...
          (min_b > 0 || max_b < 0)) {
        int64_t t0 = div_imp(min_a, min_b), t1 = div_imp(min_a, max_b),
                t2 = div_imp(max_a, min_b), t3 = div_imp(max_a, max_b);
        *min_val = std::min(std::min(t0, t1), std::min(t2, t3));
        *max_val = std::max(std::max(t0, t1), std::max(t2, t3));
        return no_overflow_scalar_int(t.element_of()) ||
               (t.can_represent(*min_val) && t.can_represent(*max_val));
  ...
  void visit(const Cast *op, const Expr &self) {
    ...
    if (value.type() == op->type) {
    ...
    } else if (cast && op->type.code() == cast->type.code() &&
    ...
    } else if (cast && (op->type.is_int() || op->type.is_uint()) &&
      ...
      expr = mutate(Broadcast::make(
          Cast::make(op->type.element_of(), broadcast_value->value),
          broadcast_value->lanes));
    } else if (ramp_value && op->type.element_of() == Int(64) &&
    ...
    if (var_info.contains(op)) {
      ...
      if (info.replacement.defined()) {
        internal_assert(info.replacement.type() == op->type)
            << "Cannot replace variable " << op->name_hint << " of type "
            << op->type << " with expression of type "
            << info.replacement.type() << "\n";
  ...
  void visit(const Add *op, const Expr &self) {
    ...
    if ((is_simple_const(a) && !is_simple_const(b)) ||
        (b.as<Min>() && !a.as<Min>()) || (b.as<Max>() && !a.as<Max>())) {
      std::swap(a, b);
    }
    ...
    if (const_int(a, &ia) && const_int(b, &ib)) {
      ...
      expr = b;
    } else if (shuffle_a && shuffle_b && shuffle_a->is_slice() &&
               shuffle_b->is_slice()) {
      ...
      expr = mutate(Select::make(
          select_a->condition, select_a->true_value + select_b->true_value,
          select_a->false_value + select_b->false_value));
    } else if (select_a && is_simple_const(b) &&
               (is_simple_const(select_a->true_value) ||
                is_simple_const(select_a->false_value))) {
      // select(c, c1, c2) + c3 -> select(c, c1+c3, c2+c3)
      expr = mutate(Select::make(select_a->condition, select_a->true_value + b,
    ...
    } else if (sub_a && add_b_a && equal(b, add_b_a->b)) {
      // (a - (b + c)) + c -> a - b
    ...
    } else if (mul_b && !is_const(a) && equal(a, mul_b->a) &&
               no_overflow(op->type)) {
      // a + a*b -> a*(1 + b)
      expr = mutate(a * (make_one(op->type) + mul_b->b));
    } else if (mul_b && !is_const(a) && equal(a, mul_b->b) &&
               no_overflow(op->type)) {
      // a + b*a -> (1 + b)*a
      expr = mutate((make_one(op->type) + mul_b->a) * a);
    } else if (mul_a && !is_const(b) && equal(mul_a->a, b) &&
               no_overflow(op->type)) {
      // a*b + a -> a*(b + 1)
      expr = mutate(mul_a->a * (mul_a->b + make_one(op->type)));
    } else if (mul_a && !is_const(b) && equal(mul_a->b, b) &&
               no_overflow(op->type)) {
      // a*b + b -> (a + 1)*b
      expr = mutate((mul_a->a + make_one(op->type)) * b);
    } else if (no_overflow(op->type) && min_a && sub_a_b &&
               equal(sub_a_b->b, b)) {
      // min(a, b-c) + c -> min(a+c, b)
      expr = mutate(Min::make(Add::make(min_a->a, b), sub_a_b->a));
    } else if (no_overflow(op->type) && min_a && sub_a_a &&
               equal(sub_a_a->b, b)) {
      // min(a-c, b) + c -> min(a, b+c)
      expr = mutate(Min::make(sub_a_a->a, Add::make(min_a->b, b)));
    } else if (no_overflow(op->type) && max_a && sub_a_b &&
               equal(sub_a_b->b, b)) {
      // max(a, b-c) + c -> max(a+c, b)
      expr = mutate(Max::make(Add::make(max_a->a, b), sub_a_b->a));
    } else if (no_overflow(op->type) && max_a && sub_a_a &&
               equal(sub_a_a->b, b)) {
      // max(a-c, b) + c -> max(a, b+c)
      expr = mutate(Max::make(sub_a_a->a, Add::make(max_a->b, b)));

    } else if (no_overflow(op->type) && min_a && add_a_b &&
               const_int(add_a_b->b, &ia) && const_int(b, &ib) &&
               ia + ib == 0) {
      // min(a, b + (-2)) + 2 -> min(a + 2, b)
      expr = mutate(Min::make(Add::make(min_a->a, b), add_a_b->a));
    } else if (no_overflow(op->type) && min_a && add_a_a &&
               const_int(add_a_a->b, &ia) && const_int(b, &ib) &&
               ia + ib == 0) {
      // min(a + (-2), b) + 2 -> min(a, b + 2)
      expr = mutate(Min::make(add_a_a->a, Add::make(min_a->b, b)));
    } else if (no_overflow(op->type) && max_a && add_a_b &&
               const_int(add_a_b->b, &ia) && const_int(b, &ib) &&
               ia + ib == 0) {
      // max(a, b + (-2)) + 2 -> max(a + 2, b)
      expr = mutate(Max::make(Add::make(max_a->a, b), add_a_b->a));
    } else if (no_overflow(op->type) && max_a && add_a_a &&
               const_int(add_a_a->b, &ia) && const_int(b, &ib) &&
               ia + ib == 0) {
      // max(a + (-2), b) + 2 -> max(a, b + 2)
      expr = mutate(Max::make(add_a_a->a, Add::make(max_a->b, b)));
    } else if (min_a && max_b && equal(min_a->a, max_b->a) &&
               equal(min_a->b, max_b->b)) {
      // min(x, y) + max(x, y) -> x + y
      expr = mutate(min_a->a + min_a->b);
    } else if (min_a && max_b && equal(min_a->a, max_b->b) &&
               equal(min_a->b, max_b->a)) {
      // min(x, y) + max(y, x) -> x + y
      expr = mutate(min_a->a + min_a->b);
    } else if (no_overflow(op->type) && div_a && add_a_a &&
               const_int(add_a_a->b, &ia) && const_int(div_a->b, &ib) && ib &&
               const_int(b, &ic)) {
      // ((a + ia) / ib + ic) -> (a + (ia + ib*ic)) / ib
      expr = mutate((add_a_a->a + IntImm::make(op->type, ia + ib * ic)) /
                    div_a->b);
      ...
      // (x%3) + y*3 -> y*3 + x%3
      expr = mutate(b + a);
    } else if (no_overflow(op->type) && mul_a && mod_b && div_a_a &&
               equal(mul_a->b, div_a_a->b) && equal(mul_a->b, mod_b->b) &&
               equal(div_a_a->a, mod_b->a)) {
      // (x/3)*3 + x%3 -> x
      expr = div_a_a->a;
    } else if (no_overflow(op->type) && add_a && mul_a_a && mod_b &&
               equal(mul_a_a->b, mod_b->b) &&
               (!mod_a_b || !equal(mod_a_b->b, mod_b->b))) {
      // ((x*3) + y) + z%3 -> (x*3 + z%3) + y
      expr = mutate((add_a->a + b) + add_a->b);
    } else if (no_overflow(op->type) && add_a && mod_a_a && mul_b &&
               equal(mod_a_a->b, mul_b->b) &&
               (!mod_a_b || !equal(mod_a_b->b, mul_b->b))) {
      // ((x%3) + y) + z*3 -> (z*3 + x%3) + y
      expr = mutate((b + add_a->a) + add_a->b);
    } else if (no_overflow(op->type) && add_a && mul_a_b && mod_b &&
               equal(mul_a_b->b, mod_b->b) &&
               (!mod_a_a || !equal(mod_a_a->b, mod_b->b))) {
      // (y + (x*3)) + z%3 -> y + (x*3 + z%3)
      expr = mutate(add_a->a + (add_a->b + b));
    } else if (no_overflow(op->type) && add_a && mod_a_b && mul_b &&
               equal(mod_a_b->b, mul_b->b) &&
               (!mod_a_a || !equal(mod_a_a->b, mul_b->b))) {
      // (y + (x%3)) + z*3 -> y + (z*3 + x%3)
      expr = mutate(add_a->a + (b + add_a->b));
    } else if (mul_a && mul_b && const_int(mul_a->b, &ia) &&
               const_int(mul_b->b, &ib) && ((ia % ib == 0) || (ib % ia == 0))) {
      // x*4 + y*2 -> (x*2 + y)*2
      // x*2 + y*4 -> (x + y*2)*2
      if (ia % ib == 0) {
      ...
      // If we've made no changes, and can't find a rule to apply, return the
      // operator unchanged.
      expr = self;
    } else {
      expr = Add::make(a, b);
  ...
  void visit(const Sub *op, const Expr &self) {
    ...
    if (is_zero(b)) {
      ...
      // Broadcast - Ramp
      expr = mutate(Ramp::make(
          broadcast_a->value - ramp_b->base,
          make_zero(ramp_b->stride.type()) - ramp_b->stride, ramp_b->lanes));
    } else if (broadcast_a && broadcast_b) {
      // Broadcast + Broadcast
      expr = Broadcast::make(mutate(broadcast_a->value - broadcast_b->value),
                             broadcast_a->lanes);
      ...
      expr = mutate(Select::make(select_a->condition, make_zero(op->type),
      ...
      expr = mutate(Select::make(select_b->condition, make_zero(op->type),
    ...
    } else if (no_overflow(op->type) && min_b && add_b_a &&
               no_overflow(op->type) && equal(a, add_b_a->a)) {
      ...
      expr = mutate(max(0 - add_b_a->b, a - min_b->b));
               ...
               no_overflow(op->type) && equal(a, add_b_a->b)) {
      // a - min(b + a, c) -> max(-b, a-c)
      expr = mutate(max(0 - add_b_a->a, a - min_b->b));
    } else if (no_overflow(op->type) && min_b && add_b_b &&
               equal(a, add_b_b->a)) {
      ...
      expr = mutate(max_a->b - max_b->a);
               ...
               (op->type.is_int() || op->type.is_uint()) && mul_a && div_a_a &&
               is_positive_const(mul_a->b) && equal(mul_a->b, div_a_a->b) &&
               equal(div_a_a->a, b)) {
      // (x/4)*4 - x -> -(x%4)
      expr = mutate(make_zero(a.type()) - (b % mul_a->b));
    } else if (no_overflow(op->type) &&
               (op->type.is_int() || op->type.is_uint()) && mul_b && div_b_a &&
               is_positive_const(mul_b->b) && equal(mul_b->b, div_b_a->b) &&
               equal(div_b_a->a, a)) {
      // x - (x/4)*4 -> x%4
      expr = mutate(a % mul_b->b);
    } else if (div_a && div_b && is_positive_const(div_a->b) &&
               equal(div_a->b, div_b->b) && op->type.is_int() &&
               no_overflow(op->type) && add_a_a && add_b_a &&
               equal(add_a_a->a, add_b_a->a) &&
               (is_simple_const(add_a_a->b) || is_simple_const(add_b_a->b))) {
      ...
      expr = mutate(((x % c) - a) / c);
               ...
               equal(div_a->b, div_b->b) && op->type.is_int() &&
               no_overflow(op->type) && sub_a_a && add_b_a &&
               equal(sub_a_a->a, add_b_a->a) && is_simple_const(add_b_a->b)) {
      ...
      expr = mutate((((x + (b % c)) % c) - a - b) / c);
               ...
               equal(div_a->b, div_b->b) && op->type.is_int() &&
               no_overflow(op->type) && add_a_a && sub_b_a &&
               equal(add_a_a->a, sub_b_a->a) && is_simple_const(add_a_a->b)) {
  ...
  void visit(const Mul *op, const Expr &self) {
    ...
    if (is_zero(a)) {
      ...
      expr = mutate(
          Ramp::make(ramp_a->base * m, ramp_a->stride * m, ramp_a->lanes));
    } else if (broadcast_a && ramp_b) {
      Expr m = broadcast_a->value;
      expr = mutate(
          Ramp::make(m * ramp_b->base, m * ramp_b->stride, ramp_b->lanes));
    } else if (add_a && !(add_a->b.as<Ramp>() && ramp_b) &&
               is_simple_const(add_a->b) && is_simple_const(b)) {
  ...
  void visit(const Div *op, const Expr &self) {
    ...
    if (const_int(b, &ib) && ib && const_int_bounds(a, &num_min, &num_max) &&
    ...
    if (is_zero(b) && !op->type.is_float()) {
      ...
      expr = mutate(
          Broadcast::make(Div::make(broadcast_a->value, broadcast_b->value),
                          broadcast_a->lanes));
    } else if (no_overflow_scalar_int(op->type) && is_const(a, -1)) {
      // -1/x -> select(x < 0, 1, -1)
      expr = mutate(select(b < make_zero(op->type), make_one(op->type),
                           make_const(op->type, -1)));
    } else if (ramp_a && no_overflow_scalar_int(ramp_a->base.type()) &&
      ...
      expr =
          mutate(Ramp::make(ramp_a->base / broadcast_b->value,
                            IntImm::make(t, div_imp(ia, ib)), ramp_a->lanes));
    } else if (ramp_a && no_overflow_scalar_int(ramp_a->base.type()) &&
               const_int(ramp_a->stride, &ia) && broadcast_b &&
               const_int(broadcast_b->value, &ib) && ib != 0 &&
               mod_rem.modulus % ib == 0 &&
               div_imp((int64_t)mod_rem.remainder, ib) ==
                   div_imp(mod_rem.remainder + (ramp_a->lanes - 1) * ia, ib)) {
      // ramp(k*z + x, y, w) / z = broadcast(k, w) if x/z == (x + (w-1)*y)/z
      expr = mutate(
          Broadcast::make(ramp_a->base / broadcast_b->value, ramp_a->lanes));
    } else if (no_overflow(op->type) && div_a && const_int(div_a->b, &ia) &&
               ia >= 0 && const_int(b, &ib) && ib >= 0) {
      // (x / 3) / 4 -> x / 12
      expr = mutate(div_a->a / make_const(op->type, ia * ib));
    } else if (no_overflow(op->type) && div_a_a && add_a &&
               const_int(div_a_a->b, &ia) && ia >= 0 &&
               const_int(add_a->b, &ib) && const_int(b, &ic) && ic >= 0) {
      // (x / ia + ib) / ic -> (x + ia*ib) / (ia*ic)
      expr = mutate((div_a_a->a + make_const(op->type, ia * ib)) /
                    make_const(op->type, ia * ic));
    } else if (no_overflow(op->type) && mul_a && const_int(mul_a->b, &ia) &&
               const_int(b, &ib) && ia > 0 && ib > 0 &&
               (ia % ib == 0 || ib % ia == 0)) {
      if (ia % ib == 0) {
        // (x * 4) / 2 -> x * 2
    ...
    } else if (no_overflow(op->type) && add_a && mul_a_a &&
               const_int(mul_a_a->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
    ...
    } else if (no_overflow(op->type) && add_a && mul_a_b &&
               const_int(mul_a_b->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // (y + x*4) / 2 -> y/2 + x*2
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((add_a->a / b) + (mul_a_b->a * ratio));
    } else if (no_overflow(op->type) && sub_a && mul_a_a &&
               const_int(mul_a_a->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // Pull terms that are a multiple of the divisor out
      // (x*4 - y) / 2 -> x*2 + (-y)/2
    ...
    } else if (no_overflow(op->type) && sub_a && mul_a_b &&
               const_int(mul_a_b->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // (y - x*4) / 2 -> y/2 - x*2
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((sub_a->a / b) - (mul_a_b->a * ratio));
    } else if (no_overflow(op->type) && add_a && add_a_a && mul_a_a_a &&
               const_int(mul_a_a_a->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // Pull terms that are a multiple of the divisor out
      // ((x*4 + y) + z) / 2 -> x*2 + (y + z)/2
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((mul_a_a_a->a * ratio) + (add_a_a->b + add_a->b) / b);
    } else if (no_overflow(op->type) && add_a && sub_a_a && mul_a_a_a &&
               const_int(mul_a_a_a->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // ((x*4 - y) + z) / 2 -> x*2 + (z - y)/2
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((mul_a_a_a->a * ratio) + (add_a->b - sub_a_a->b) / b);
    } else if (no_overflow(op->type) && sub_a && add_a_a && mul_a_a_a &&
               const_int(mul_a_a_a->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // ((x*4 + y) - z) / 2 -> x*2 + (y - z)/2
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((mul_a_a_a->a * ratio) + (add_a_a->b - sub_a->b) / b);
    } else if (no_overflow(op->type) && sub_a && sub_a_a && mul_a_a_a &&
               const_int(mul_a_a_a->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // ((x*4 - y) - z) / 2 -> x*2 + (0 - y - z)/2
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((mul_a_a_a->a * ratio) + (-sub_a_a->b - sub_a->b) / b);
    } else if (no_overflow(op->type) && add_a && add_a_b && mul_a_b_a &&
               const_int(mul_a_b_a->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // (x + (y*4 + z)) / 2 -> y*2 + (x + z)/2
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((mul_a_b_a->a * ratio) + (add_a->a + add_a_b->b) / b);
    } else if (no_overflow(op->type) && add_a && sub_a_b && mul_a_b_a &&
               const_int(mul_a_b_a->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // (x + (y*4 - z)) / 2 -> y*2 + (x - z)/2
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((mul_a_b_a->a * ratio) + (add_a->a - sub_a_b->b) / b);
    } else if (no_overflow(op->type) && sub_a && add_a_b && mul_a_b_a &&
               const_int(mul_a_b_a->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // (x - (y*4 + z)) / 2 -> (x - z)/2 - y*2
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((sub_a->a - add_a_b->b) / b - (mul_a_b_a->a * ratio));
    } else if (no_overflow(op->type) && add_a && sub_a_b && mul_a_b_b &&
               const_int(mul_a_b_b->b, &ia) && const_int(b, &ib) && ib > 0 &&
               (ia % ib == 0)) {
      // (x - (z*4 - y)) / 2 -> (x + (y - z*4)) / 2  -- by a rule from Sub
      // (x + (y - z*4)) / 2 -> (x + y)/2 - z*2  -- by this rule
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((add_a->a + sub_a_b->a) / b - (mul_a_b_b->a * ratio));
    } else if (no_overflow(op->type) && add_a && const_int(add_a->b, &ia) &&
               const_int(b, &ib) && ib > 0 && (ia % ib == 0)) {
      // (y + 8) / 2 -> y/2 + 4
      Expr ratio = make_const(op->type, div_imp(ia, ib));
      expr = mutate((add_a->a / b) + ratio);
    } else if (no_overflow(op->type) && osub_a && omod_a_b &&
               const_int(omod_a_b->b, &ia) && const_int(b, &ib) &&
               equal(osub_a->a, omod_a_b->a) && ib > 0 && ia == ib) {
      // (x - x%2)/2 -> x/2
    ...
    } else if (no_overflow(op->type) && sub_a && !is_zero(b) &&
               equal(sub_a->a, b)) {
      // (x - y)/x -> (-y)/x + 1
  ...
  void visit(const Mod *op, const Expr &self) {
    ...
    if (broadcast_b && const_int(broadcast_b->value, &ib) && ib && ramp_a &&
    ...
    if (is_zero(b) && !op->type.is_float()) {
      ...
      expr = mutate(
          Broadcast::make(Mod::make(broadcast_a->value, broadcast_b->value),
                          broadcast_a->lanes));
    } else if (no_overflow(op->type) && mul_a && const_int(b, &ib) && ib &&
               const_int(mul_a->b, &ia) && (ia % ib == 0)) {
      // (x * (b*a)) % b -> 0
      expr = make_zero(op->type);
    } else if (no_overflow(op->type) && add_a && mul_a_a &&
               const_int(mul_a_a->b, &ia) && const_int(b, &ib) && ib &&
               (ia % ib == 0)) {
      // (x * (b*a) + y) % b -> (y % b)
      ...
      expr = mutate(add_a->a % b);
               ...
               const_int(b, &ib) && ib && (ia >= ib || ia <= -ib)) {
      // (y + c) % b -> ((y + c%b) % b)
      expr = mutate((add_a->a + IntImm::make(Int(32), ia % ib)) % b);
    } else if (no_overflow(op->type) && add_a && mul_a_b &&
               const_int(mul_a_b->b, &ia) && const_int(b, &ib) && ib &&
               (ia % ib == 0)) {
      // (y + x * (b*a)) % b -> (y % b)
      expr = mutate(add_a->a % b);
    } else if (no_overflow(op->type) && add_a && add_a_a && mul_a_a_b &&
               const_int(mul_a_a_b->b, &ia) && const_int(b, &ib) && ib &&
               (ia % ib == 0)) {
      // ((y + x * (b*a)) + z) % b -> ((y + z) % b)
      expr = mutate((add_a_a->a + add_a->b) % b);
    } else if (no_overflow_scalar_int(op->type) && const_int(b, &ib) && ib &&
               mod_rem.modulus % ib == 0) {
      // ((a*b)*x + c) % a -> c % a
      ...
      Expr new_base = make_const(ramp_a->base.type(),
                                 mod_imp((int64_t)mod_rem.remainder, ib));
  ...
  void visit(const Min *op, const Expr &self) {
    ...
    if (equal(a, b)) {
      ...
      // Compute minimum of expression of type and maximum of type -->
      // expression
      expr = a;
      return;
    } else if (const_int(b, &ib) && b.type().is_min(ib)) {
      // Compute minimum of expression of type and minimum of type --> min of
      // type
      expr = b;
      return;
      ...
      expr = mutate(
          Broadcast::make(Min::make(broadcast_a->value, broadcast_b->value),
                          broadcast_a->lanes));
      return;
    ...
    } else if (no_overflow(op->type) && ramp_a && broadcast_b &&
    ...
    if (no_overflow(op->type) && add_a && const_int(add_a->b, &ia) && add_b &&
        const_int(add_b->b, &ib) && equal(add_a->a, add_b->a)) {
      // min(x + 3, x - 2) -> x - 2
      if (ia > ib) {
        expr = b;
      } else {
    ...
    } else if (no_overflow(op->type) && add_b && const_int(add_b->b, &ib) &&
               equal(add_b->a, a)) {
      // min(x, x + 5) -> x
      if (ib > 0) {
        expr = a;
    ...
    } else if (no_overflow(op->type) && sub_a && sub_b &&
               equal(sub_a->b, sub_b->b) && const_int(sub_a->a, &ia) &&
      ...
      // min(((a + 3)/4)*4, a) -> a
      expr = b;
    } else if (a_round_up.defined() && max_b && equal(a_round_up, max_b->a) &&
      ...
      // min(min(x, broadcast(y, n)), broadcast(z, n))) -> min(x,
      // broadcast(min(y, z), n))
      expr = mutate(Min::make(
          min_a->a,
          Broadcast::make(Min::make(broadcast_a_b->value, broadcast_b->value),
                          broadcast_b->lanes)));
      ...
      // min(min(min(min(min(x, y), z), w), l), y) -> min(min(min(min(x, y), z),
      // w), l)
      expr = a;
    ...
    } else if (no_overflow(op->type) && add_a && add_b &&
    ...
    } else if (no_overflow(op->type) && div_a && div_b &&
               const_int(div_a->b, &ia) && ia && const_int(div_b->b, &ib) &&
               (ia == ib)) {
    ...
    } else if (no_overflow(op->type) && mul_a && mul_b &&
               const_int(mul_a->b, &ia) && const_int(mul_b->b, &ib) &&
               (ia == ib)) {
      Expr factor = make_const(op->type, ia);
      if (ia > 0) {
    ...
    } else if (call_a && call_a->is_intrinsic(Call::likely) &&
               equal(call_a->args[0], b)) {
      // min(likely(b), b) -> likely(b)
      expr = a;
    } else if (call_b && call_b->is_intrinsic(Call::likely) &&
    ...
    } else if (no_overflow(op->type) && sub_a && is_const(sub_a->a) &&
               is_const(b)) {
      // min(8 - x, 3) -> 8 - max(x, 5)
  ...
  void visit(const Max *op, const Expr &self) {
    ...
    if (equal(a, b)) {
      ...
      // Compute maximum of expression of type and minimum of type -->
      // expression
      expr = a;
      return;
    } else if (const_int(b, &ib) && b.type().is_max(ib)) {
      // Compute maximum of expression of type and maximum of type --> max of
      // type
      expr = b;
      return;
    } else if (op->type.is_uint() && is_zero(b)) {
      ...
      expr = mutate(
          Broadcast::make(Max::make(broadcast_a->value, broadcast_b->value),
                          broadcast_a->lanes));
      return;
    } else if (const_int_bounds(a, &a_min, &a_max) &&
               const_int_bounds(b, &b_min, &b_max)) {
    ...
    if (no_overflow(op->type) && add_a && const_int(add_a->b, &ia) && add_b &&
      ...
      // max(max(x, broadcast(y, n)), broadcast(z, n))) -> max(x,
      // broadcast(max(y, z), n))
      expr = mutate(Max::make(
          max_a->a,
      ...
      // max(max(max(max(max(x, y), z), w), l), y) -> max(max(max(max(x, y), z),
      // w), l)
      expr = a;
    } else if (max_a && max_b && equal(max_a->a, max_b->a)) {
      // Distributive law for min/max
    ...
    } else if (no_overflow(op->type) && sub_a && is_simple_const(sub_a->a) &&
               is_simple_const(b)) {
      // max(8 - x, 3) -> 8 - min(x, 5)
  ...
  void visit(const EQ *op, const Expr &self) {
    ...
    if (broadcast) {
      ...
      expr = Broadcast::make(
          mutate(broadcast->value == make_zero(broadcast->value.type())),
          broadcast->lanes);
    ...
    } else if (sel && (is_positive_const(sel->true_value) ||
                       is_negative_const(sel->true_value))) {
      // select(c, 4, f) == 0 -> !c && (f == 0)
      expr = mutate((!sel->condition) && (sel->false_value == zero));
    } else if (sel && is_zero(sel->false_value)) {
      // select(c, t, 0) == 0 -> !c || (t == 0)
      expr = mutate((!sel->condition) || (sel->true_value == zero));
    } else if (sel && (is_positive_const(sel->false_value) ||
                       is_negative_const(sel->false_value))) {
  ...
  void visit(const LT *op, const Expr &self) {
    ...
    if (const_int(a, &ia) && const_int(b, &ib)) {
      ...
      // Comparing maximum of type < expression of type.  This can never be
      // true.
      expr = const_false(op->type.lanes());
    } else if (const_int(b, &ib) && b.type().is_min(ib)) {
      // Comparing expression of type < minimum of type.  This can never be
      // true.
      expr = const_false(op->type.lanes());
    } else if (is_zero(delta) ||
               (no_overflow(delta.type()) && is_positive_const(delta))) {
      expr = const_false(op->type.lanes());
    } else if (no_overflow(delta.type()) && is_negative_const(delta)) {
      expr = const_true(op->type.lanes());
    } else if (broadcast_a && broadcast_b) {
      // Push broadcasts outwards
      expr = mutate(Broadcast::make(broadcast_a->value < broadcast_b->value,
                                    broadcast_a->lanes));
      ...
      if (ramp_a && ramp_b && equal(ramp_a->stride, ramp_b->stride)) {
        ...
        // Rearrange so that all adds and subs are on the rhs to cut down on
        // further cases
        expr = mutate(add_a->a < (b - add_a->b));
      } else if (sub_a) {
        expr = mutate(sub_a->a < (b + sub_a->b));
      ...
      } else if (sub_b && is_const(a) && is_const(sub_b->a) &&
                 !is_const(sub_b->b)) {
        // (c1 < c2 - x) -> (x < c2 - c1)
        expr = mutate(sub_b->b < (sub_b->a - a));
      } else if (mul_a && mul_b && is_positive_const(mul_a->b) &&
                 is_positive_const(mul_b->b) && equal(mul_a->b, mul_b->b)) {
      ...
      } else if (a.type().is_int() && div_a && is_positive_const(div_a->b) &&
                 is_const(b)) {
        // a / c1 < c2 <=> a < c1*c2
        expr = mutate(div_a->a < (div_a->b * b));
      } else if (a.type().is_int() && div_b && is_positive_const(div_b->b) &&
                 is_const(a)) {
        // c1 < b / c2 <=> (c1+1)*c2-1 < b
      ...
      } else if (mul_a && div_a_a && const_int(div_a_a->b, &ia) &&
                 const_int(mul_a->b, &ib) && ia > 0 && ia == ib &&
                 equal(div_a_a->a, b)) {
        // subtract (x/c1)*c1 from both sides
        // (x/c1)*c1 < x -> 0 < x % c1
        expr = mutate(0 < b % make_const(a.type(), ia));
      } else if (mul_a && div_a_a && add_b && const_int(div_a_a->b, &ia) &&
                 const_int(mul_a->b, &ib) && ia > 0 && ia == ib &&
                 equal(div_a_a->a, add_b->a)) {
        // subtract (x/c1)*c1 from both sides
        // (x/c1)*c1 < x + y -> 0 < x % c1 + y
        expr = mutate(0 < add_b->a % div_a_a->b + add_b->b);
      } else if (mul_a && div_a_a && sub_b && const_int(div_a_a->b, &ia) &&
                 const_int(mul_a->b, &ib) && ia > 0 && ia == ib &&
                 equal(div_a_a->a, sub_b->a)) {
        // subtract (x/c1)*c1 from both sides
        // (x/c1)*c1 < x - y -> y < x % c1
        expr = mutate(sub_b->b < sub_b->a % div_a_a->b);
      } else if (mul_a && div_a_a && add_a_a_a && const_int(div_a_a->b, &ia) &&
                 const_int(mul_a->b, &ib) && const_int(add_a_a_a->b, &ic) &&
                 ia > 0 && ia == ib && equal(add_a_a_a->a, b)) {
        // subtract ((x+c2)/c1)*c1 from both sides
        // ((x+c2)/c1)*c1 < x -> c2 < (x+c2) % c1
        expr = mutate(add_a_a_a->b < div_a_a->a % div_a_a->b);
      } else if (mul_a && div_a_a && add_b && add_a_a_a &&
                 const_int(div_a_a->b, &ia) && const_int(mul_a->b, &ib) &&
                 const_int(add_a_a_a->b, &ic) && ia > 0 && ia == ib &&
                 equal(add_a_a_a->a, add_b->a)) {
        // subtract ((x+c2)/c1)*c1 from both sides
        // ((x+c2)/c1)*c1 < x + y -> c2 < (x+c2) % c1 + y
        expr = mutate(add_a_a_a->b < div_a_a->a % div_a_a->b + add_b->b);
      } else if (mul_a && div_a_a && add_a_a_a && sub_b &&
                 const_int(div_a_a->b, &ia) && const_int(mul_a->b, &ib) &&
                 const_int(add_a_a_a->b, &ic) && ia > 0 && ia == ib &&
        ...
        expr = mutate(sub_b->b <
                      div_a_a->a % div_a_a->b + make_const(a.type(), -ic));
      } else if (delta_ramp && is_positive_const(delta_ramp->stride) &&
                 is_one(mutate(delta_ramp->base + delta_ramp->stride *
                                                      (delta_ramp->lanes - 1) <
                               0))) {
        ...
        expr = const_false(delta_ramp->lanes);
                 ...
                 ia * (delta_ramp->lanes - 1) + mod_rem.remainder <
                     mod_rem.modulus) {
        // ramp(x, a, b) < 0 -> broadcast(x < 0, b)
        expr = Broadcast::make(
            mutate(LT::make(delta_ramp->base / mod_rem.modulus, 0)),
            delta_ramp->lanes);
      } else if (a.same_as(op->a) && b.same_as(op->b)) {
        expr = self;
      } else {
  ...
  void visit(const GT *op, const Expr &self) { expr = mutate(op->b < op->a); }
  ...
  void visit(const And *op, const Expr &self) {
    ...
    if (is_one(a)) {
      ...
      // a && !a
      expr = const_false(op->type.lanes());
    } else if (le_a && lt_b && equal(le_a->a, lt_b->b) &&
               equal(le_a->b, lt_b->a)) {
      // a <= b && b < a
      expr = const_false(op->type.lanes());
    } else if (lt_a && le_b && equal(lt_a->a, le_b->b) &&
               equal(lt_a->b, le_b->a)) {
      // a < b && b <= a
      expr = const_false(op->type.lanes());
    } else if (lt_a && lt_b && equal(lt_a->a, lt_b->b) &&
               const_int(lt_a->b, &ia) && const_int(lt_b->a, &ib) &&
               ib + 1 >= ia) {
      // (a < ia && ib < a) where there is no integer a s.t. ib < a < ia
      expr = const_false(op->type.lanes());
    } else if (lt_a && lt_b && equal(lt_a->b, lt_b->a) &&
               const_int(lt_b->b, &ia) && const_int(lt_a->a, &ib) &&
               ib + 1 >= ia) {
      // (ib < a && a < ia) where there is no integer a s.t. ib < a < ia
      expr = const_false(op->type.lanes());

    } else if (le_a && lt_b && equal(le_a->a, lt_b->b) &&
               const_int(le_a->b, &ia) && const_int(lt_b->a, &ib) && ib >= ia) {
      // (a <= ia && ib < a) where there is no integer a s.t. ib < a <= ia
      expr = const_false(op->type.lanes());
    } else if (le_a && lt_b && equal(le_a->b, lt_b->a) &&
               const_int(lt_b->b, &ia) && const_int(le_a->a, &ib) && ib >= ia) {
      // (ib <= a && a < ia) where there is no integer a s.t. ib < a <= ia
      expr = const_false(op->type.lanes());

    } else if (lt_a && le_b && equal(lt_a->a, le_b->b) &&
               const_int(lt_a->b, &ia) && const_int(le_b->a, &ib) && ib >= ia) {
      // (a < ia && ib <= a) where there is no integer a s.t. ib <= a < ia
      expr = const_false(op->type.lanes());
    } else if (lt_a && le_b && equal(lt_a->b, le_b->a) &&
               const_int(le_b->b, &ia) && const_int(lt_a->a, &ib) && ib >= ia) {
      // (ib < a && a <= ia) where there is no integer a s.t. ib <= a < ia
      expr = const_false(op->type.lanes());

    } else if (le_a && le_b && equal(le_a->a, le_b->b) &&
               const_int(le_a->b, &ia) && const_int(le_b->a, &ib) && ib > ia) {
      // (a <= ia && ib <= a) where there is no integer a s.t. ib <= a <= ia
      expr = const_false(op->type.lanes());
    } else if (le_a && le_b && equal(le_a->b, le_b->a) &&
               const_int(le_b->b, &ia) && const_int(le_a->a, &ib) && ib > ia) {
      // (ib <= a && a <= ia) where there is no integer a s.t. ib <= a <= ia
      expr = const_false(op->type.lanes());

    } else if (eq_a && neq_b && equal(eq_a->a, neq_b->a) &&
               is_simple_const(eq_a->b) && is_simple_const(neq_b->b)) {
      // (a == k1) && (a != k2) -> (a == k1) && (k1 != k2)
      // (second term always folds away)
      expr = mutate(And::make(a, NE::make(eq_a->b, neq_b->b)));
    } else if (neq_a && eq_b && equal(neq_a->a, eq_b->a) &&
               is_simple_const(neq_a->b) && is_simple_const(eq_b->b)) {
      // (a != k1) && (a == k2) -> (a == k2) && (k1 != k2)
      // (second term always folds away)
      expr = mutate(And::make(b, NE::make(neq_a->b, eq_b->b)));
    } else if (eq_a && eq_a->a.as<Variable>() && is_simple_const(eq_a->b) &&
               expr_uses_var(b, eq_a->a.as<Variable>())) {
      // (somevar == k) && b -> (somevar == k) && substitute(somevar, k, b)
      expr =
          mutate(And::make(a, substitute(eq_a->a.as<Variable>(), eq_a->b, b)));
    } else if (eq_b && eq_b->a.as<Variable>() && is_simple_const(eq_b->b) &&
               expr_uses_var(a, eq_b->a.as<Variable>())) {
      // a && (somevar == k) -> substitute(somevar, k1, a) && (somevar == k)
      expr =
          mutate(And::make(substitute(eq_b->a.as<Variable>(), eq_b->b, a), b));
    } else if (broadcast_a && broadcast_b &&
               broadcast_a->lanes == broadcast_b->lanes) {
      // x8(a) && x8(b) -> x8(a && b)
      expr = Broadcast::make(
          mutate(And::make(broadcast_a->value, broadcast_b->value)),
          broadcast_a->lanes);
  ...
  void visit(const Or *op, const Expr &self) {
    ...
    if (is_one(a)) {
      ...
      expr = const_true(op->type.lanes());
               ...
               const_int(lt_a->b, &ia) && const_int(lt_b->a, &ib) && ib < ia) {
      // (a < ia || ib < a) where ib < ia
      expr = const_true(op->type.lanes());
    } else if (lt_a && lt_b && equal(lt_a->b, lt_b->a) &&
               const_int(lt_b->b, &ia) && const_int(lt_a->a, &ib) && ib < ia) {
      ...
      expr = const_true(op->type.lanes());
               ...
               const_int(le_a->b, &ia) && const_int(le_b->a, &ib) &&
               ib <= ia + 1) {
      // (a <= ia || ib <= a) where ib <= ia + 1
      expr = const_true(op->type.lanes());
    } else if (le_a && le_b && equal(le_a->b, le_b->a) &&
               const_int(le_b->b, &ia) && const_int(le_a->a, &ib) &&
               ib <= ia + 1) {
      ...
      // x8(a) || x8(b) -> x8(a || b)
      expr = Broadcast::make(
          mutate(Or::make(broadcast_a->value, broadcast_b->value)),
          broadcast_a->lanes);
    } else if (eq_a && neq_b && equal(eq_a->a, neq_b->a) &&
               is_simple_const(eq_a->b) && is_simple_const(neq_b->b)) {
      // (a == k1) || (a != k2) -> (a != k2) || (k1 == k2)
    ...
    } else if (is_var_simple_const_comparison(b, &xvar_c) && and_a &&
               ((is_var_simple_const_comparison(and_a->a, &xvar_a) &&
                 xvar_a == xvar_c) ||
                (is_var_simple_const_comparison(and_a->b, &xvar_b) &&
                 xvar_b == xvar_c))) {
      // (a && b) || (c) -> (a || c) && (b || c)
    ...
    } else if (is_var_simple_const_comparison(a, &xvar_c) && and_b &&
               ((is_var_simple_const_comparison(and_b->a, &xvar_a) &&
                 xvar_a == xvar_c) ||
                (is_var_simple_const_comparison(and_b->b, &xvar_b) &&
                 xvar_b == xvar_c))) {
      // (c) || (a && b) -> (a || c) && (b || c)
      // iff c and at least one of a or b is of the form
      //     (var == const) or (var != const)
  ...
  void visit(const Select *op, const Expr &self) {
    ...
    if (propagate_indeterminate_expression(condition, true_value, false_value,
                                           op->type, &expr)) {
      return;
    }
    ...
    if (is_zero(condition)) {
      ...
      expr = true_value;
    } else if (true_value.type().is_bool() && is_one(true_value) &&
               is_zero(false_value)) {
    ...
    } else if (true_value.type().is_bool() && is_zero(true_value) &&
               is_one(false_value)) {
      if (true_value.type().is_vector() && condition.type().is_scalar()) {
      ...
      expr = true_value;
    } else if (cf && cf->is_intrinsic(Call::likely) &&
      ...
      expr = mutate(add_t->a +
                    Select::make(condition, add_t->b,
                                 make_zero(sub_f->b.type()) - sub_f->b));
    } else if (add_t && sub_f && equal(add_t->b, sub_f->a)) {
      // select(c, b+a, a-d) -> a + select(x, b, 0-d)
      expr = mutate(add_t->b +
                    Select::make(condition, add_t->a,
                                 make_zero(sub_f->b.type()) - sub_f->b));
    } else if (sub_t && add_f && equal(sub_t->a, add_f->a)) {
      // select(c, a-b, a+d) -> a + select(x, 0-b, d)
      expr =
          mutate(sub_t->a + Select::make(condition,
                                         make_zero(sub_t->b.type()) - sub_t->b,
                                         add_f->b));
  ...
  void visit(const IfThenElse *op, const Stmt &self) {
    ...
    // Mine the condition for useful constraints to apply (eg var == value &&
    // bool_param).
    vector<Expr> stack;
  ...
  void visit(const Load *op, const Expr &self) {
    ...
    if (is_zero(predicate)) {
      ...
      Expr load = Load::make(op->type.element_of(), op->buffer_var,
                             b_index->value, b_pred->value);
  ...
  void visit(const Call *op, const Expr &self) {
    // Calls implicitly depend on host, dev, mins, and strides of the buffer
    // referenced
      ...
      */
      ...
      if (const double *f = as_const_float(arg)) {
        if (op->name == "floor_f32" || op->name == "floor") {
          ...
          expr = FloatImm::make(arg.type(),
                                (*f < 0 ? std::ceil(*f) : std::floor(*f)));
        }
  ...
  void visit(const Shuffle *op, const Expr &self) {
    if (op->is_extract_element() &&
        (op->vectors[0].as<Ramp>() || op->vectors[0].as<Broadcast>())) {
    ...
    if (op->is_interleave()) {
      ...
      if (r) {
        ...
        if (can_collapse) {
          expr =
              Ramp::make(r->base, mutate(r->stride / terms), r->lanes * terms);
          return;
        }
      }
        ...
        if (first_shuffle->is_slice()) {
          ...
          for (size_t i = 0; i < new_vectors.size() && can_collapse; i++) {
            ...
            if (i > 0) {
              ...
              for (size_t j = 0;
                   j < first_shuffle->vectors.size() && can_collapse; j++) {
      ...
      if (r) {
        ...
        for (size_t i = 1; i < new_vectors.size() && can_collapse; i++) {
          Expr diff;
          if (new_vectors[i].type().lanes() ==
              new_vectors[i - 1].type().lanes()) {
            diff = mutate(new_vectors[i] - new_vectors[i - 1]);
          }

          const Broadcast *b = diff.as<Broadcast>();
          if (b) {
            Expr check = mutate(b->value -
                                r->stride * new_vectors[i - 1].type().lanes());
            can_collapse &= is_zero(check);
          } else {
            can_collapse = false;
          }
      ...
      if (new_vectors[0].type().is_scalar() &&
          new_vectors[1].type().is_scalar()) {
        bool can_collapse = true;
  ...
  Expr hoist_slice_vector(Expr e) {
    ...
    internal_assert(shuffle_a && shuffle_b && shuffle_a->is_slice() &&
                    shuffle_b->is_slice());
  ...
  Body simplify_let(const T *op, const Body &self) {
    ...
    // Iteratively peel off certain operations from the let value and push them
    // inside.
    ...
    debug(4) << "simplify let " << op->var << " = " << value << " in ... "
             << op->var << " ...\n";
    ...
    while (1) {
      ...
      if (is_const(new_value)) {
        ...
        replacement =
            substitute(new_var, Add::make(new_var, add->b), replacement);
        new_value = add->a;
      } else if (mul && (is_const(mul->b) || var_b)) {
        replacement =
            substitute(new_var, Mul::make(new_var, mul->b), replacement);
        new_value = mul->a;
      } else if (div && is_const(div->b)) {
        replacement =
            substitute(new_var, Div::make(new_var, div->b), replacement);
        new_value = div->a;
      } else if (sub && (is_const(sub->b) || var_b)) {
        replacement =
            substitute(new_var, Sub::make(new_var, sub->b), replacement);
        new_value = sub->a;
      } else if (mod && is_const(mod->b)) {
        replacement =
            substitute(new_var, Mod::make(new_var, mod->b), replacement);
        new_value = mod->a;
      } else if (min && is_const(min->b)) {
        replacement =
            substitute(new_var, Min::make(new_var, min->b), replacement);
        new_value = min->a;
      } else if (max && is_const(max->b)) {
        replacement =
            substitute(new_var, Max::make(new_var, max->b), replacement);
        new_value = max->a;
      } else if (ramp && is_const(ramp->stride)) {
        new_value = ramp->base;
        VarExpr repl_var = Variable::make(new_value.type(), new_name);
        replacement =
            substitute(new_var, Ramp::make(repl_var, ramp->stride, ramp->lanes),
                       replacement);
        new_var = repl_var;
      } else if (broadcast) {
        ...
        replacement = substitute(
            new_var, Broadcast::make(repl_var, broadcast->lanes), replacement);
        new_var = repl_var;
        ...
        VarExpr repl_var = Variable::make(new_value.type(), new_name);
        replacement =
            substitute(new_var, Cast::make(cast->type, repl_var), replacement);
        new_var = repl_var;
        ...
        VarExpr repl_var = Variable::make(new_value.type(), new_name);
        replacement = substitute(
            new_var, Shuffle::make({repl_var}, slice_indices), replacement);
        new_var = repl_var;
      } else if (shuffle && shuffle->is_concat() &&
                 ((var_a && !var_b) || (!var_a && var_b))) {
        VarExpr repl_var = Variable::make(
            var_a ? shuffle->vectors[1].type() : shuffle->vectors[0].type(),
            new_name);
        Expr op_a = var_a ? shuffle->vectors[0] : repl_var;
        Expr op_b = var_a ? repl_var : shuffle->vectors[1];
        replacement = substitute(new_var, Shuffle::make_concat({op_a, op_b}),
                                 replacement);
    ...
    if (new_value.same_as(value)) {
      ...
      debug(4) << "new let " << new_name << " = " << new_value << " in ... "
               << replacement << " ...\n";
    }
    ...
    if (new_op && new_op->var.same_as(op->var) &&
        new_op->body.same_as(op->body) && new_op->value.same_as(op->value)) {
      return self;
    }
  ...
  void visit(const AssertStmt *op, const Stmt &self) {
    ...
    if (a && is_zero(a->condition)) {
      ...
      if (!const_false_conditions_expected) {
        user_warning
            << "This pipeline is guaranteed to fail an assertion at runtime: \n"
            << stmt << "\n";
      }
  ...
  void visit(const For *op, const Stmt &self) {
    ...
    if (is_no_op(new_body)) {
      stmt = new_body;
    } else if (op->min.same_as(new_min) && op->extent.same_as(new_extent) &&
               op->body.same_as(new_body)) {
      stmt = self;
    } else {
      stmt = For::make(op->loop_var, new_min, new_extent, op->for_type,
  ...
  void visit(const Store *op, const Stmt &self) {
    ...
    if (is_zero(predicate)) {
      ...
      stmt = IfThenElse::make(scalar_pred->value,
                              Store::make(op->buffer_var, value, index,
                                          const_true(value.type().lanes())));
    } else if (is_undef(value) ||
               (load && load->buffer_var.same_as(op->buffer_var) &&
                equal(load->index, index))) {
      // foo[x] = foo[x] or foo[x] = undef is a no-op
      stmt = Evaluate::make(0);
    } else if (predicate.same_as(op->predicate) && value.same_as(op->value) &&
  ...
  void visit(const Allocate *op, const Stmt &self) {
    ...
    if (body_if && op->condition.defined() &&
        equal(op->condition, body_if->condition)) {
      // We can move the allocation into the if body case. The
      // else case must not use it.
      stmt = Allocate::make(op->buffer_var, op->type, new_extents, condition,
                            body_if->then_case, op->attrs, new_expr,
      ...
      stmt = Allocate::make(op->buffer_var, op->type, new_extents, condition,
                            body, op->attrs, new_expr, op->free_function,
  ...
  void visit(const Block *op, const Stmt &self) {
    ...
    if (is_no_op(first) && is_no_op(rest)) {
      ...
      // Do both first and rest start with the same let statement (occurs when
      // unrolling).
    ...
    } else if (if_first && if_rest && !if_rest->else_case.defined() &&
               expr_is_pure(if_first->condition) &&
               expr_is_pure(if_rest->condition) &&
               is_one(mutate((if_first->condition && if_rest->condition) ==
                             if_rest->condition))) {
  ...
  void visit(const GetBit *op, const Expr &self) {
    ...
    if (op->a.same_as(a) && op->index.same_as(index)) {
      expr = self;
    } else {
  ...
  void visit(const GetSlice *op, const Expr &self) {
    ...
    } else if (op->a.same_as(a) && op->index_left.same_as(index_left) &&
  ...
  void visit(const SetBit *op, const Expr &self) {
    ...
    if (op->a.same_as(a) && op->value.same_as(value) &&
  ...
  void visit(const SetSlice *op, const Expr &self) {
    ...
    } else if (op->a.same_as(a) && op->value.same_as(value) &&
...
Expr simplify(Expr e, bool simplify_lets, const Scope<Interval> &bounds,
              const Scope<ModulusRemainder> &alignment) {
  return Simplify(simplify_lets, &bounds, &alignment).mutate(e);
}

Stmt simplify(Stmt s, bool simplify_lets, const Scope<Interval> &bounds,
              const Scope<ModulusRemainder> &alignment) {
...
class SimplifyExprs : public IRMutator {
 public:
  using IRMutator::mutate;
  Expr mutate(Expr e) { return simplify(e); }
};

Stmt simplify_exprs(Stmt s) { return SimplifyExprs().mutate(s); }
...
Expr concat_vectors(const vector<Expr> &e) { return Shuffle::make_concat(e); }
...
Expr broadcast(const Expr &base, int w) { return Broadcast::make(base, w); }

VarExpr Var(string name) { return Variable::make(Int(32), name); }
...
void check_casts() {
  ...
  check(min(cast(Int(16), 30000), cast(Int(16), -123)),
        make_const(Int(16), -123));
  check(max(cast(Int(16), 30000), cast(Int(16), 65000)),
        make_const(Int(16), 30000));
  ...
  // Specific checks for 32 bit unsigned expressions - ensure simplifications
  // are actually unsigned. 4000000000 (4 billion) is less than 2^32 but more
  // than 2^31.  As an int, it is negative.
  check(cast(UInt(32), (int)4000000000UL) + cast(UInt(32), 5),
        make_const(UInt(32), (int)4000000005UL));
  check(cast(UInt(32), (int)4000000000UL) - cast(UInt(32), 5),
        make_const(UInt(32), (int)3999999995UL));
  check(cast(UInt(32), (int)4000000000UL) / cast(UInt(32), 5),
        make_const(UInt(32), 800000000));
  check(cast(UInt(32), 800000000) * cast(UInt(32), 5),
        make_const(UInt(32), (int)4000000000UL));
  check(cast(UInt(32), (int)4000000023UL) % cast(UInt(32), 100),
        make_const(UInt(32), 23));
  check(min(cast(UInt(32), (int)4000000023UL), cast(UInt(32), 1000)),
        make_const(UInt(32), (int)1000));
  check(max(cast(UInt(32), (int)4000000023UL), cast(UInt(32), 1000)),
        make_const(UInt(32), (int)4000000023UL));
  check(cast(UInt(32), (int)4000000023UL) < cast(UInt(32), 1000),
        const_false());
  check(cast(UInt(32), (int)4000000023UL) == cast(UInt(32), 1000),
        const_false());
...
void check_algebra() {
  ...
  check(Expr(broadcast(4.0f, 5)) + Expr(ramp(3.25f, 4.5f, 5)),
        ramp(7.25f, 4.5f, 5));
  check(Expr(ramp(3.25f, 4.5f, 5)) + Expr(broadcast(4.0f, 5)),
        ramp(7.25f, 4.5f, 5));
  ...
  check((3 * x) * 4, x * 12);
  check(4 * (3 + x), x * 4 + 12);
  check(Expr(broadcast(4.0f, 5)) * Expr(ramp(3.0f, 4.0f, 5)),
        ramp(12.0f, 16.0f, 5));
  check(Expr(ramp(3.0f, 4.0f, 5)) * Expr(broadcast(2.0f, 5)),
        ramp(6.0f, 8.0f, 5));
...
void check_vectors() {
  ...
  check(Expr(ramp(x, 2, 4)) % (broadcast(2, 4)), broadcast(x % 2, 4));
  check(Expr(ramp(2 * x + 1, 4, 4)) % (broadcast(2, 4)), broadcast(1, 4));

  check(ramp(0, 1, 4) == broadcast(2, 4), ramp(-2, 1, 4) == broadcast(0, 4));

  {
    Expr test =
        select(ramp(const_true(), const_true(), 2),
               ramp(const_false(), const_true(), 2),
               broadcast(const_false(), 2)) == broadcast(const_false(), 2);
    Expr expected =
        !(ramp(const_true(), const_true(), 2)) ||
  ...
  {
    Expr test =
        select(ramp(const_true(), const_true(), 2), broadcast(const_true(), 2),
               ramp(const_false(), const_true(), 2)) ==
...
void check_bounds() {
  ...
  check(min(broadcast(x, 4), broadcast(y, 4)), broadcast(min(x, y), 4));
  check(min(x, x + 3), x);
  check(min(x + 4, x), x);
  ...
  check(max(broadcast(x, 4), broadcast(y, 4)), broadcast(max(x, y), 4));
  check(max(x, x + 3), x + 3);
  check(max(x + 4, x), x + 4);
  ...
  // the other expression.  The result of < or > comparison is known to be true
  // or false.
  ...
  check(max(cast(Int(8), x), cast(Int(8), -127)),
        max(cast(Int(8), x), make_const(Int(8), -127)));
...

  {
    ...
    std::vector<Expr> clamped = {
        max(min(v1, four), -four), max(-four, min(v1, four)),
        min(max(v1, -four), four), min(four, max(v1, -four)),
        clamp(v1, -four, four)};
    ...
    // There is simplification rule that will simplify max(max(min(x, 4), -4),
    // 3) further into max(min(x, 4), 3)
    check(max(min(clamped[0], five), three),
          simplify(max(min(v1, four), three)));
    ...
    for (size_t i = 0; i < clamped.size(); ++i) {
      ...
      check(max(min(clamped[i] + one, four), -four),
            simplify(min(clamped[i] + one, four)));
    }
    for (size_t i = 0; i < clamped.size(); ++i) {
      ...
      check(min(max(clamped[i] + one, -four), four),
            simplify(min(clamped[i] + one, four)));
    }
...

  {
    ...
    // min(min(x, broadcast(y, n)), broadcast(z, n))) -> min(x, broadcast(min(y,
    // z), n))
    check(min(min(xv, broadcast(y, 64)), broadcast(z, 64)),
          min(xv, broadcast(min(y, z), 64)));
    // min(min(broadcast(x, n), y), broadcast(z, n))) -> min(y, broadcast(min(x,
    // z), n))
    check(min(min(broadcast(x, 64), yv), broadcast(z, 64)),
          min(yv, broadcast(min(x, z), 64)));
    // min(broadcast(x, n), min(y, broadcast(z, n)))) -> min(y, broadcast(min(x,
    // z), n))
    check(min(broadcast(x, 64), min(yv, broadcast(z, 64))),
          min(yv, broadcast(min(z, x), 64)));
    // min(broadcast(x, n), min(broadcast(y, n), z))) -> min(z, broadcast(min(x,
    // y), n))
    check(min(broadcast(x, 64), min(broadcast(y, 64), zv)),
          min(zv, broadcast(min(y, x), 64)));

    // max(max(x, broadcast(y, n)), broadcast(z, n))) -> max(x, broadcast(max(y,
    // z), n))
    check(max(max(xv, broadcast(y, 64)), broadcast(z, 64)),
          max(xv, broadcast(max(y, z), 64)));
    // max(max(broadcast(x, n), y), broadcast(z, n))) -> max(y, broadcast(max(x,
    // z), n))
    check(max(max(broadcast(x, 64), yv), broadcast(z, 64)),
          max(yv, broadcast(max(x, z), 64)));
    // max(broadcast(x, n), max(y, broadcast(z, n)))) -> max(y, broadcast(max(x,
    // z), n))
    check(max(broadcast(x, 64), max(yv, broadcast(z, 64))),
          max(yv, broadcast(max(z, x), 64)));
    // max(broadcast(x, n), max(broadcast(y, n), z))) -> max(z, broadcast(max(x,
    // y), n))
    check(max(broadcast(x, 64), max(broadcast(y, 64), zv)),
          max(zv, broadcast(max(y, x), 64)));
  }
}
...
void check_boolean() {
  ...
  check(!(x != y), x == y);
  check(!(!(x == 0)), x == 0);
  check(!Expr(broadcast(x > y, 4)), broadcast(x <= y, 4));
...

  {
    ...
    check((i != 2 && (i != 4 && (i != 8 && i != 16))) || (i == 16),
          (i != 2 && (i != 4 && (i != 8))));
    check((i == 16) || (i != 2 && (i != 4 && (i != 8 && i != 16))),
          (i != 2 && (i != 4 && (i != 8))));
  }
  ...
  check(ramp(x * 8 + 8, 1, 4) < broadcast(y * 8, 4),
        broadcast(x < y + (-1), 4));
  check(ramp(x * 8 + 5, 1, 4) < broadcast(y * 8, 4),
        ramp(x * 8 + 5, 1, 4) < broadcast(y * 8, 4));
  check(ramp(x * 8 - 1, 1, 4) < broadcast(y * 8, 4),
  ...
  check(ramp(x * 8 + 2, 2, 4) < broadcast(y * 8, 4),
        ramp(x * 8 + 2, 2, 4) < broadcast(y * 8, 4));
  check(ramp(x * 8, 3, 4) < broadcast(y * 8, 4),
        ramp(x * 8, 3, 4) < broadcast(y * 8, 4));
  check(select(ramp((x / 16) * 16, 1, 8) < broadcast((y / 8) * 8, 8),
               broadcast(1, 8), broadcast(3, 8)),
        select((x / 16) * 2 < y / 8, broadcast(1, 8), broadcast(3, 8)));

  check(ramp(x * 8, -1, 4) < broadcast(y * 8, 4),
  ...
  check(IfThenElse::make(x == 4 && y == 5, Evaluate::make(z + x + y),
                         Evaluate::make(z + x - y)),
        IfThenElse::make(x == 4 && y == 5, Evaluate::make(z + 9),
                         Evaluate::make(z + x - y)));
  ...
  check(IfThenElse::make(b1, Evaluate::make(select(b1, x, y)),
                         Evaluate::make(select(b1, z, w))),
        IfThenElse::make(b1, Evaluate::make(x), Evaluate::make(w)));

  check(
      IfThenElse::make(
          x < y, IfThenElse::make(x < y, Evaluate::make(y), Evaluate::make(x)),
          Evaluate::make(x)),
  ...
  check(Block::make(IfThenElse::make(x < y, Evaluate::make(x + 1),
                    ...
                    IfThenElse::make(x < y, Evaluate::make(x + 3),
                                     Evaluate::make(x + 4))),
        IfThenElse::make(
            x < y, Block::make(Evaluate::make(x + 1), Evaluate::make(x + 3)),
            Block::make(Evaluate::make(x + 2), Evaluate::make(x + 4))));

  check(Block::make(IfThenElse::make(x < y, Evaluate::make(x + 1)),
                    IfThenElse::make(x < y, Evaluate::make(x + 2))),
        IfThenElse::make(
            x < y, Block::make(Evaluate::make(x + 1), Evaluate::make(x + 2))));
  ...
  check(IfThenElse::make(foo == 17, Evaluate::make(x + foo + 1),
                         Evaluate::make(x + foo + 2)),
        IfThenElse::make(foo_simple == 17, Evaluate::make(x + 18),
                         Evaluate::make(x + foo_simple + 2)));

  check(IfThenElse::make(foo != 17, Evaluate::make(x + foo + 1),
                         Evaluate::make(x + foo + 2)),
        IfThenElse::make(foo_simple != 17, Evaluate::make(x + foo_simple + 1),
                         Evaluate::make(x + 19)));
...

  {
    ...
    check(select(b[0], x, select(b[1], x, y)), select(b[0] || b[1], x, y));
    check(select(b[0], x, select(b[1], y, x)), select(b[0] || !b[1], x, y));
    check(select(b[0], select(b[1], x, y), x), select(b[0] && !b[1], y, x));
    check(select(b[0], select(b[1], y, x), x), select(b[0] && b[1], y, x));
...
void check_ind_expr(Expr e, bool expect_error) {
  ...
  if (expect_error && !is_error)
    internal_error << "Expression should be indeterminate: " << e
                   << " but saw: " << e2 << "\n";
  else if (!expect_error && is_error)
    internal_error << "Expression should not be indeterminate: " << e
                   << " but saw: " << e2 << "\n";
}
...
void check_indeterminate() {
  const int32_t values[] = {
      int32_t(0x80000000), -2147483647, -2, -1, 0, 1, 2, 2147483647,
  };
    ...
    for (int32_t i2 : values) {
      {
        ...
        bool r_is_zero =
            !i1 || (i2 != 0 &&
                    !div_imp((int64_t)i1,
                             (int64_t)i2));  // avoid trap for -2147483648/-1
        bool r_is_ind = !i2;
...
void simplify_test() {
  ...
  check(Evaluate::make(
            Let::make(x, Call::make(Int(32), "dummy", {3, x, 4}, Call::Extern),
                      Let::make(y, 10, x + y + 2))),
        LetStmt::make(x, Call::make(Int(32), "dummy", {3, x, 4}, Call::Extern),
                      Evaluate::make(x + 12)));

  // Test case with most negative 32-bit number, as constant to check that it is
  // not negated.
  ...
  check(Call::make(type_of<const char *>(), Call::stringify,
                   {3, string(" "), 4}, Call::Intrinsic),
        string("3 4"));

  check(Call::make(type_of<const char *>(), Call::stringify,
                   {3, x, 4, string(", "), 3.4f}, Call::Intrinsic),
        Call::make(type_of<const char *>(), Call::stringify,
                   {string("3"), x, string("4, 3.400000")}, Call::Intrinsic));
  ...
  // Check if we can simplify away comparison on vector types considering
  // bounds.
  ...
  check_in_bounds(ramp(x, -1, 4) < broadcast(-4, 4), const_false(4),
                  bounds_info);
  check_in_bounds(ramp(x, -1, 4) < broadcast(5, 4), const_true(4), bounds_info);
  check_in_bounds(min(ramp(x, 1, 4), broadcast(0, 4)), broadcast(0, 4),
                  bounds_info);
  check_in_bounds(min(ramp(x, 1, 4), broadcast(8, 4)), ramp(x, 1, 4),
                  bounds_info);
  check_in_bounds(min(ramp(x, -1, 4), broadcast(-4, 4)), broadcast(-4, 4),
                  bounds_info);
  check_in_bounds(min(ramp(x, -1, 4), broadcast(5, 4)), ramp(x, -1, 4),
                  bounds_info);
  check_in_bounds(max(ramp(x, 1, 4), broadcast(0, 4)), ramp(x, 1, 4),
                  bounds_info);
  check_in_bounds(max(ramp(x, 1, 4), broadcast(8, 4)), broadcast(8, 4),
                  bounds_info);
  check_in_bounds(max(ramp(x, -1, 4), broadcast(-4, 4)), ramp(x, -1, 4),
                  bounds_info);
  check_in_bounds(max(ramp(x, -1, 4), broadcast(5, 4)), broadcast(5, 4),
                  bounds_info);
  ...
  check(interleave_vectors({ramp(x - y, 2 * y, 4), ramp(x, 2 * y, 4)}),
        ramp(x - y, y, 8));
  check(
      interleave_vectors({ramp(x, 3, 4), ramp(x + 1, 3, 4), ramp(x + 2, 3, 4)}),
      ramp(x, 1, 12));
  {
    Expr vec = ramp(x, 1, 16);
    check(interleave_vectors({slice(vec, 0, 2, 8), slice(vec, 1, 2, 8)}), vec);
    check(interleave_vectors({slice(vec, 0, 4, 4), slice(vec, 1, 4, 4),
                              slice(vec, 2, 4, 4), slice(vec, 3, 4, 4)}),
          vec);
  }

  // Collapse some vector concats
  check(concat_vectors({ramp(x, 2, 4), ramp(x + 8, 2, 4)}), ramp(x, 2, 8));
  check(concat_vectors({ramp(x, 3, 2), ramp(x + 6, 3, 2), ramp(x + 12, 3, 2)}),
        ramp(x, 3, 6));
  ...
  // Now check that an interleave of some collapsible loads collapses into a
  // single dense load
  {
    VarExpr buf = Var("buf"), buf2 = Var("buf2");
    Expr load1 = Load::make(Float(32, 4), buf, ramp(x, 2, 4), const_true(4));
    Expr load2 =
        Load::make(Float(32, 4), buf, ramp(x + 1, 2, 4), const_true(4));
    ...
    Expr load3 =
        Load::make(Float(32, 4), buf2, ramp(x + 1, 2, 4), const_true(4));
    e = interleave_vectors({load1, load3});
    check(e, e);
  }

  // Check that concatenated loads of adjacent scalars collapse into a vector
  // load.
  {
    VarExpr buf = Var("buf");
    int lanes = 4;
    ...
    check(concat_vectors(loads),
          Load::make(Float(32, lanes), buf, ramp(x, 1, lanes),
                     const_true(lanes)));
  }
  ...
  {
    Expr e = Broadcast::make(-16, 2) <
             (ramp(Cast::make(UInt(16), 7), Cast::make(UInt(16), 11), 2) -
              Broadcast::make(1, 2));
    Expr expected =
        Broadcast::make(-16, 2) <
        (ramp(make_const(UInt(16), 7), make_const(UInt(16), 11), 2) -
         Broadcast::make(1, 2));
    check(e, expected);
  }

  {
    ...
    Expr value =
        Load::make(index.type(), f, index, const_true(index.type().lanes()));
...

  {
    ...
    // Exprs, rather than being promoted to int first. (TODO: This doesn't
    // really belong in the test for Simplify, but IROperator has no test unit
    // of its own.)
    ...
    // Explicitly passing 'two' as an Expr, rather than an int, will defeat this
    // logic.
...

  {
    ...
    Expr e = simplify(max(ramp(x, y, 2), broadcast(x, 2)) -
                      max(broadcast(y, 2), ramp(y, y, 2)));
    Expr expected = max(ramp(x, y, 2), broadcast(x, 2)) -
                    max(ramp(y, y, 2), broadcast(y, 2));
    check(e, expected);
  }
