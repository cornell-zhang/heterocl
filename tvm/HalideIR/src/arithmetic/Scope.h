...
class Scope {
 ...
 public:
  ...
  T &ref(const Variable *var) {
    typename std::map<const Variable *, SmallStack<T>>::iterator iter =
        table.find(var);
    if (iter == table.end() || iter->second.empty()) {
      internal_error << "Symbol '" << var->name_hint << "' not found\n";
    }
   ...
   */
  void push(const Variable *var, const T &value) { table[var].push(value); }
  ...
  void pop(const Variable *var) {
    typename std::map<const Variable *, SmallStack<T>>::iterator iter =
        table.find(var);
    internal_assert(iter != table.end())
        << "Name not in symbol table: " << var->name_hint << "\n";
    iter->second.pop();
  ...
  class const_iterator {
   ...
   public:
    explicit const_iterator(
        const typename std::map<const Variable *, SmallStack<T>>::const_iterator
            &i)
        : iter(i) {}
    ...
    const Variable *var() { return iter->first; }

    const SmallStack<T> &stack() { return iter->second; }

    const T &value() { return iter->second.top_ref(); }
  };
