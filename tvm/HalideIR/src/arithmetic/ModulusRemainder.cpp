...
void check(Expr e, int m, int r) {
  ...
  if (result.modulus != m || result.remainder != r) {
    ...
    std::cerr << "Computed modulus, remainder = " << result.modulus << ", "
              << result.remainder << "\n";
    exit(-1);
  }
}
...
void ComputeModulusRemainder::visit(const Select *op, const Expr &) {
  ModulusRemainder r =
      unify_alternatives(analyze(op->true_value), analyze(op->false_value));
  modulus = r.modulus;
  remainder = r.remainder;
}
...
void ComputeModulusRemainder::visit(const Shuffle *op, const Expr &) {
  // It's possible that scalar expressions are extracting a lane of a vector -
  // don't fail in this case, but stop
