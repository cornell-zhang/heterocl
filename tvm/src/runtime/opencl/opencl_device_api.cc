...
const std::shared_ptr<OpenCLWorkspace>& OpenCLWorkspace::Global() {
  static std::shared_ptr<OpenCLWorkspace> inst =
      std::make_shared<OpenCLWorkspace>();
  return inst;
}
...
void OpenCLWorkspace::GetAttr(TVMContext ctx, DeviceAttrKind kind,
                              TVMRetValue* rv) {
  this->Init();
  ...
  switch (kind) {
    case kMaxThreadsPerBlock: {
      OPENCL_CALL(clGetDeviceInfo(devices[index], CL_DEVICE_MAX_WORK_GROUP_SIZE,
...
void* OpenCLWorkspace::AllocDataSpace(TVMContext ctx, size_t size,
                                      size_t alignment, TVMType type_hint) {
  this->Init();
  CHECK(context != nullptr) << "No OpenCL device";
  cl_int err_code;
  cl_mem mptr = clCreateBuffer(this->context, CL_MEM_READ_WRITE, size, nullptr,
                               &err_code);
...
void OpenCLWorkspace::CopyDataFromTo(const void* from, size_t from_offset,
  ...
  if (ctx_from.device_type == kDLOpenCL && ctx_to.device_type == kDLOpenCL) {
    OPENCL_CALL(
        clEnqueueCopyBuffer(this->GetQueue(ctx_to),
                            static_cast<cl_mem>((void*)from),  // NOLINT(*)
                            static_cast<cl_mem>(to), from_offset, to_offset,
                            size, 0, nullptr, nullptr));
  } else if (ctx_from.device_type == kDLOpenCL &&
             ctx_to.device_type == kDLCPU) {
    OPENCL_CALL(clEnqueueReadBuffer(
        this->GetQueue(ctx_from),
        static_cast<cl_mem>((void*)from),  // NOLINT(*)
        CL_FALSE, from_offset, size, static_cast<char*>(to) + to_offset, 0,
        nullptr, nullptr));
    OPENCL_CALL(clFinish(this->GetQueue(ctx_from)));
  } else if (ctx_from.device_type == kDLCPU &&
             ctx_to.device_type == kDLOpenCL) {
    OPENCL_CALL(clEnqueueWriteBuffer(
        this->GetQueue(ctx_to), static_cast<cl_mem>(to), CL_FALSE, to_offset,
        size, static_cast<const char*>(from) + from_offset, 0, nullptr,
        nullptr));
...
bool MatchPlatformInfo(cl_platform_id pid, cl_platform_info param_name,
                       std::string value) {
...
void OpenCLWorkspace::Init() {
  ...
  if (devices_matched.size() == 0) {
    LOG(WARNING)
        << "No OpenCL device any device matched given the options: gpu mode";
    LOG(WARNING) << "Now try OpenCL cpu mode";
    devices_matched = cl::GetDeviceIDs(this->platform_id, "cpu");
    if (devices_matched.size() == 0) {
      LOG(WARNING)
          << "No OpenCL device any device matched given the options: cpu mode";
      return;
    }
  }
  ...
  cl_int err_code;
  this->context =
      clCreateContext(nullptr, this->devices.size(), &(this->devices[0]),
