...
void CodeGenAOCLHost::VisitStmt_(const KernelStmt* op) {
  ...
  size_t serialized_buffer_num = 0;
  bool is_all_buffers_serialized = false;
  ...
  for (size_t i = 0; i < op->annotate_keys.size(); i++) {
    ...
    if (ends_with(arg_name, "dev.deser") || ends_with(arg_name, "dev.ser")) {
      serialized_buffer_num += 1;
      serialized_buffers.insert(arg_name);
      HCL_DEBUG_LEVEL(2) << "[AOCL host] serialized buffer " << arg_name;
    }
  ...
  // If all buffers are (de)serialized, then we skip top kernel generation
  // addition kernel calls are generated by (de)serialization instrinsics
  if (serialized_buffer_num == args_info.size()) {
    is_all_buffers_serialized = true;
  }

  if (args_info.size() > 0) {
    // Create kernels
    stream << "\n";
    if (!is_all_buffers_serialized) {
      PrintIndent();
    ...
    // If all the interface buffers are serialized, then we laucn kernels for each 
    // IO (de)serialization kernels on FPGA
    } else {
      // Create OpenCL kernels
      for (auto name: serialized_buffers) {
        std::string kernel_name;
        std::string attr = ".dev.ser";
        if (ends_with(name, attr)) {
          kernel_name = name + "_IO_L3_in_serialize";
          kernel_name.replace(kernel_name.find(attr), attr.length(), "");
        } else {
          attr = ".dev.deser";
          kernel_name = name + "_drain_IO_L3_out_serialize";
          kernel_name.replace(kernel_name.find(attr), attr.length(), "");
        }
        legalize_name(name);
        legalize_name(kernel_name);
        stream << "  cl_kernel kernel_" << name << " = clCreateKernel(program, \"" 
               << kernel_name << "\", &status); CHECK(status);\n";
      ...
      for (auto name: serialized_buffers) {
        legalize_name(name);
        PrintIndent();
        stream << "status = clSetKernelArg(kernel_"
               << name << ", 0, sizeof(cl_mem), (void *)&buffer_"
               << name << "); CHECK(status);\n";
        PrintIndent();
        stream << "status = clEnqueueNDRangeKernel(cmdQueue_"
               << name << ", kernel_" << name
               << ", 1, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL); CHECK(status);\n";
      }
      ...
      for (auto name: serialized_buffers) {
        legalize_name(name);
        PrintIndent();
        stream << "status = clFlush(cmdQueue_" << name << "); CHECK(status);\n";
        PrintIndent();
        stream << "status = clFinish(cmdQueue_" << name << "); CHECK(status);\n";
      }
    }
