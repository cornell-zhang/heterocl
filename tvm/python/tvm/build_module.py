...
def build_fpga_kernel(flist, target_name):
    """Build an FPGA kernel.

    Parameters
    ----------
    flist : list of LoweredFunc or Stmt
        The list generated by `build`.
    ...
    FIXME: This is inconsistent with `build` return type.
    """
...
def build(sch,
    ...
    if "fpga" in target.keys:
        return build_fpga_kernel(flist, target.target_name)
