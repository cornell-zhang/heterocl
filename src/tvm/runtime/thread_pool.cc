...
class ParallelLauncher {
 public:
  ...
  void Init(FTVMParallelLambda flambda, void* cdata, int num_task,
            bool need_sync) {
  ...
  int WaitForJobs() {
    ...
    cv_.wait(lock, [this] { return num_pending_ == 0; });
...
class ParallelTaskQueue {
 public:
  ...
  ParallelTaskQueue() { ring_.resize(2); }
  /*!
  ...
  bool Pop(Task* task, int timeout = 10) {
    ...
    if (num_pending_ != 0) {
      *task = ring_[head_];
      ...
      lock.lock();
      ++nwait_consumer_;
      cv_.wait(lock, [this] { return num_pending_ != 0 || exit_now_.load(); });
      --nwait_consumer_;
      *task = ring_[head_];
      head_ = (head_ + 1) % ring_.size();
      --num_pending_;
...
class ThreadPool {
 ...
 private:
  ...
  void Init() {
    ...
    for (int i = 0; i < num_workers_; ++i) {
      threads_[i] =
          std::thread([this, i] { this->RunWorker(queues_[i].get()); });
    }
  }
