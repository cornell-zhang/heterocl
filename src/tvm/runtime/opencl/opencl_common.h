 ...
 */
#ifndef RUNTIME_OPENCL_OPENCL_COMMON_H_
#define RUNTIME_OPENCL_OPENCL_COMMON_H_
...
#define OPENCL_CHECK_ERROR(e)                                    \
  {                                                              \
    CHECK(e == CL_SUCCESS) << "OpenCL Error, code=" << e << ": " \
                           << cl::CLGetErrorString(e);           \
  }
 ...
 public:
  ...
  cl_command_queue GetQueue(TVMContext ctx) {
    ...
    CHECK(ctx.device_id >= 0 &&
          static_cast<size_t>(ctx.device_id) < queues.size())
  ...
  void* AllocDataSpace(TVMContext ctx, size_t size, size_t alignment,
...
class OpenCLThreadEntry {
 public:
  ...
  WorkspacePool pool;
  // constructor
  OpenCLThreadEntry() : pool(kDLOpenCL, OpenCLWorkspace::Global()) {
    context.device_id = 0;
...
#endif  // RUNTIME_OPENCL_OPENCL_COMMON_H_
