...
class CoProcSyncPlanner : public StorageAccessVisitor {
 public:
  explicit CoProcSyncPlanner(const std::unordered_set<const Variable*>& touched,
 ...
 private:
  ...
  std::vector<Stmt> GetSync(std::string sync_name) {
    return {
        Evaluate::make(Call::make(Int(32), sync_name, {}, Call::Intrinsic))};
  }
...
class CoProcBarrierDetector : public StorageAccessVisitor {
 ...
 private:
  ...
  std::vector<AccessEntry> PlanWriteBarrier(std::vector<StmtEntry> seq,
                                            const For* loop) {
  ...
  std::vector<AccessEntry> PlanReadBarrier(std::vector<StmtEntry> seq,
                                           const For* loop) {
...
class CoProcInstDepDetector : public IRVisitor {
 public:
  explicit CoProcInstDepDetector(const IterVar& coproc_axis,
                                 const std::string& coproc_name)
  ...
  void Visit_(const For* op) final {
    ...
    if (last_state_.node != nullptr) {
      ...
      InjectSync(last_state_, first_state_, &(curr_state_.exit_push),
  ...
  void Visit_(const IfThenElse* op) final {
    ...
    std::swap(last_state_, temp_last);
    {
      ...
      if (last_state_.node != nullptr) {
        ...
        MatchFixEnterPop(first_state_);
        MatchFixExitPush(last_state_);
        curr_state.enter_ctx.insert(first_state_.enter_ctx.begin(),
                                    first_state_.enter_ctx.end());
        curr_state.exit_ctx.insert(last_state_.exit_ctx.begin(),
 ...
 private:
  ...
  void InjectSync(const SyncState& prev, const SyncState& next,
    ...
    for (const std::pair<int, int>& p : pending) {
      if (std::find(prev.exit_push.begin(), prev.exit_push.end(), p) ==
          prev.exit_push.end()) {
        vpush.push_back(p);
        prev_after.emplace_back(MakePush(p.first, p.second));
      }
      if (std::find(next.enter_pop.begin(), next.enter_pop.end(), p) ==
          next.enter_pop.end()) {
        vpop.push_back(p);
  ...
  Stmt MakePush(int from, int to) {
    return Evaluate::make(Call::make(
        ...
        {make_const(Int(32), from), make_const(Int(32), to)}, Call::Intrinsic));
  }
 ...
 public:
  ...
  Stmt Mutate(Stmt stmt) final {
    ...
    if (it != insert_before_.end()) {
      before =
          MergeSeq(std::vector<Stmt>(it->second.rbegin(), it->second.rend()));
    }
...
Stmt CoProcSync(Stmt stmt) { return CoProcSyncInserter().Insert(stmt); }
