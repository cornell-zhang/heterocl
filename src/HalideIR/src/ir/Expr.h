...
/** \file
 * Base classes for Halide expressions (\ref Halide::Expr) and statements (\ref
...
struct IRHandle : public NodeRef {
  IRHandle() {}
  explicit IRHandle(std::shared_ptr<Node> p) : NodeRef(p) {}
...
struct ExprHash {
  size_t operator()(const Expr &a) const { return a.hash(); }
};
...
struct Stmt : public IRHandle {
  Stmt() : IRHandle() {}
  explicit Stmt(std::shared_ptr<IR::Node> n) : IRHandle(n) {}
