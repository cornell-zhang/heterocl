...
void IRVisitor::visit(const Not *op, const Expr &) { op->a.accept(this); }
...
void IRVisitor::visit(const Reuse *op, const Stmt &) { op->body.accept(this); }
...
void IRGraphVisitor::visit(const Not *op, const Expr &) { include(op->a); }
...
void IRGraphVisitor::visit(const While *op, const Stmt &) {
  include(op->condition);
  include(op->body);
}

void IRGraphVisitor::visit(const Reuse *op, const Stmt &) { include(op->body); }
