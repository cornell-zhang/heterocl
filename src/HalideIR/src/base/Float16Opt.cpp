...
float16_t::operator float() const { return half2float(data); }

float16_t::operator double() const { return half2float(data); }
...
float16_t float16_t::make_infinity(bool positive) {
  return float16_t(std::numeric_limits<float>::infinity(),
...
bool float16_t::operator>(float16_t rhs) const {
  internal_assert(!this->are_unordered(rhs))
      << "Cannot compare unorderable values\n";
  return half2float(data) > half2float(rhs.data);
}
...
bool float16_t::is_nan() const { return std::isnan(half2float(data)); }

bool float16_t::is_infinity() const { return std::isinf(half2float(data)); }

bool float16_t::is_negative() const { return half2float(data) < 0; }

bool float16_t::is_zero() const { return half2float(data) == 0; }

uint16_t float16_t::to_bits() const { return this->data; }
