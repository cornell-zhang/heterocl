...
struct halideir_cplusplus_type_name {
  /// An enum to indicate whether a C++ type is non-composite, a struct, class,
  /// or union
  enum CPPTypeType {
    Simple,         ///< "int"
    Struct,         ///< "struct Foo"
    ...
    Union,          ///< "union Foo"
    Enum,           ///< "enum Foo"
  } cpp_type_type;  // Note: order is reflected in map_to_name table in
                    // CPlusPlusMangle.cpp

  std::string name;

  halideir_cplusplus_type_name(CPPTypeType cpp_type_type,
                               const std::string &name)
      : cpp_type_type(cpp_type_type), name(name) {}

  bool operator==(const halideir_cplusplus_type_name &rhs) const {
    return cpp_type_type == rhs.cpp_type_type && name == rhs.name;
  }
  ...
  bool operator<(const halideir_cplusplus_type_name &rhs) const {
    return cpp_type_type < rhs.cpp_type_type ||
           (cpp_type_type == rhs.cpp_type_type && name < rhs.name);
  }
};
...
struct halideir_handle_cplusplus_type {
  ...
  halideir_handle_cplusplus_type(
      const halideir_cplusplus_type_name &inner_name,
...
#define HALIDEIR_DECLARE_EXTERN_TYPE(TypeType, Type)          \
  template <>                                                 \
  struct halideir_c_type_to_name<Type> {                      \
...
#define HALIDEIR_DECLARE_EXTERN_SIMPLE_TYPE(T) \
  HALIDEIR_DECLARE_EXTERN_TYPE(Simple, T)
#define HALIDEIR_DECLARE_EXTERN_STRUCT_TYPE(T) \
  HALIDEIR_DECLARE_EXTERN_TYPE(Struct, T)
#define HALIDEIR_DECLARE_EXTERN_CLASS_TYPE(T) \
  HALIDEIR_DECLARE_EXTERN_TYPE(Class, T)
#define HALIDEIR_DECLARE_EXTERN_UNION_TYPE(T) \
  HALIDEIR_DECLARE_EXTERN_TYPE(Union, T)
...
// Default case (should be only Unknown types, since we specialize for Known
// types below). We require that all unknown types be pointers, and translate
// them all to void* (preserving const-ness and volatile-ness).
template <typename T, bool KnownType>
struct halideir_internal_handle_traits {
  static const halideir_handle_cplusplus_type *type_info(
      bool is_ptr, halideir_handle_cplusplus_type::ReferenceType ref_type) {
    static_assert(!KnownType, "Only unknown types handled here");
    internal_assert(is_ptr) << "Unknown types must be pointers";
    internal_assert(ref_type == halideir_handle_cplusplus_type::NotReference)
        << "Unknown types must not be references";
    static const halideir_handle_cplusplus_type the_info{
        ...
        {(uint8_t)(halideir_handle_cplusplus_type::Pointer |
                   ...
                   (std::is_volatile<T>::value
                        ? halideir_handle_cplusplus_type::Volatile
                        : 0))},
...
struct halideir_internal_handle_traits<T, true> {
  static const halideir_handle_cplusplus_type make_info(
      bool is_ptr, halideir_handle_cplusplus_type::ReferenceType ref_type) {
    ...
    the_info.inner_name.name = Halide::Internal::extract_namespaces(
        the_info.inner_name.name, the_info.namespaces);
    return the_info;
  }
...
struct halideir_handle_traits<T *> {
  inline static const halideir_handle_cplusplus_type *type_info() {
    return halideir_internal_handle_traits<
        T,
        halideir_c_type_to_name<typename std::remove_cv<T>::type>::known_type>::
        type_info(true, halideir_handle_cplusplus_type::NotReference);
  }
};
...
struct halideir_handle_traits<T &> {
  inline static const halideir_handle_cplusplus_type *type_info() {
    return halideir_internal_handle_traits<
        T,
        halideir_c_type_to_name<typename std::remove_cv<T>::type>::known_type>::
        type_info(false, halideir_handle_cplusplus_type::LValueReference);
  }
};
...
struct halideir_handle_traits<T &&> {
  inline static const halideir_handle_cplusplus_type *type_info() {
    return halideir_internal_handle_traits<
        T,
        halideir_c_type_to_name<typename std::remove_cv<T>::type>::known_type>::
        type_info(false, halideir_handle_cplusplus_type::RValueReference);
  }
};
...
enum QuanMode { RND, RND_ZERO, RND_MIN_INF, RND_INF, RND_CONV, TRN, TRN_ZERO };

enum OverMode { SAT, SAT_ZERO, SAT_SYM, WRAP, WRAP_SM };
...
struct Type {
 ...
 public:
  ...
  Type()
      : type(Handle, 0, 0, 0), qmode(RND), omode(SAT), handle_type(nullptr) {}

  /** Construct a runtime representation of a Halide type from:
   * code: The fundamental type from an enum.
   * bits: The bit size of one element.
   * lanes: The number of vector elements in the type. */
  Type(halideir_type_code_t code, int bits, int lanes, int fracs = 0,
       QuanMode qmode = RND, OverMode omode = SAT,
       const halideir_handle_cplusplus_type *handle_type = nullptr)
      : type(code, (uint16_t)bits, (uint8_t)lanes, (uint8_t)fracs),
        qmode(RND),
        omode(SAT),
   ...
   * the runtime value. */
  Type(const halideir_type_t &that,
       const halideir_handle_cplusplus_type *handle_type = nullptr)
  ...
  /** Return Type with same number of bits and lanes, but new_code for a type
   * code. */
  ...
  /** Return Type with same type code and lanes, but new_bits for the number of
   * bits. */
  ...
  /** Return Type with same type code and lanes, but new_fracs for the number of
   * frac bits. */
  ...
  bool operator==(const Type &other) const {
    return code() == other.code() && bits() == other.bits() &&
           lanes() == other.lanes() && fracs() == other.fracs() &&
  ...
  Type element_of() const { return with_lanes(1); }
...
inline Type Fixed(int bits, int fracs, int lanes = 1, QuanMode qmode = RND,
                  OverMode omode = SAT) {
  return Type(Type::Int, bits, lanes, fracs, qmode, omode);
}

/** Constructing a unsigned fixed-point type */
inline Type UFixed(int bits, int fracs, int lanes = 1, QuanMode qmode = RND,
                   OverMode omode = SAT) {
