...
void set_custom_compile_time_error_reporter(
...
ErrorReport::ErrorReport(const char *file, int line,
                         const char *condition_string, int flags)
    : flags(flags) {
  const std::string &source_loc = "";

  if (flags & User) {
    // Only mention where inside of libHalide the error tripped if we have debug
    // level > 0
...
#endif
{
  ...
  // TODO(seanlatias): Add an option to error out on warnings too
  if (flags & Warning) {
    std::cerr << msg.str();
    return;
  }
