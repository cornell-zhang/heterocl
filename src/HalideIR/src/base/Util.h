...
// by default, the symbol EXPORT does nothing. In windows dll builds we can
// define it to __declspec(dllexport)
#if defined(_WIN32)
...
inline NO_INLINE void collect_paired_args(
    std::vector<std::pair<T1, T2>> &collected_args, const T3 &a1,
    const T4 &a2) {
...
inline NO_INLINE void collect_paired_args(
    std::vector<std::pair<T1, T2>> &collected_args, const T3 &a1, const T4 &a2,
    Args &&... args) {
  collected_args.push_back(std::pair<T1, T2>(a1, a2));
...
struct meta_and<T1, Args...>
    : std::integral_constant<bool, T1::value && meta_and<Args...>::value> {};
...
struct meta_or<T1, Args...>
    : std::integral_constant<bool, T1::value || meta_or<Args...>::value> {};

template <typename To, typename... Args>
struct all_are_convertible : meta_and<std::is_convertible<Args, To>...> {};

/** Returns base name and fills in namespaces, outermost one first in vector. */
EXPORT std::string extract_namespaces(const std::string &name,
...
using make_integer_sequence =
    typename make_integer_sequence_helper<T, 0, N>::type;
