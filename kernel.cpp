...
void default_function(unsigned char* seqAs, unsigned char* seqBs, unsigned char* outAs, unsigned char* outBs) {
  int B;
  for (int task_outer = 0; task_outer < 32; ++task_outer) {
    for (int task_inner = 0; task_inner < 32; ++task_inner) {
      int maxtrix_max;
      maxtrix_max = 0;
      int i_max;
      i_max = 0;
      ...
      for (int i = 0; i < 129; ++i) {
        for (int j = 0; j < 129; ++j) {
          ...
          if ((i != 0) && (j != 0)) {
            int result;
            result = -4;
            if (seqAs[((i + (((task_outer * 32) + task_inner) * 128)) + -1)] == seqBs[((j + (((task_outer * 32) + task_inner) * 128)) + -1)]) {
              result = 1;
            }
            trace_back[0] = ((int)(((long)matrix[((j + (i * 129)) + -130)]) + ((long)result)));
      ...
      while (((curr_i != next_i) || (curr_j != next_j))) {
        ...
        if (next_i == curr_i) {
          ...
          a = ((int)seqAs[((curr_i + (((task_outer * 32) + task_inner) * 128)) + -1)]);
        }
        if (next_j == curr_j) {
          b = 0;
        } else {
          b = ((int)seqBs[((curr_j + (((task_outer * 32) + task_inner) * 128)) + -1)]);
        }
        outAs[(tick + (((task_outer * 32) + task_inner) * 256))] = ((unsigned char)a);
        outBs[(tick + (((task_outer * 32) + task_inner) * 256))] = ((unsigned char)b);
        curr_i = next_i;
        curr_j = next_j;
        int act2;
