...
def test_with_if():
    ...
    def kernel(matrix_1, matrix_2):
        return_matrix = hcl.compute(
            (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y], "return_matrix"
        )

        with hcl.if_(matrix_2[0, 0] == 0):
            hcl.assert_(
                matrix_2[1, 1] == 0, "assert message in if statement"
            )  # result is true
            hcl.print(0, "in the if statement\n")  # should be printed

        hcl.assert_(
            matrix_1[0, 0] != 0, "customized assert message 1"
        )  # result is false
...
def test_with_for():
    ...
    def kernel(matrix_1, matrix_2):
        ...
        with hcl.for_(0, 7, name="for_loop") as f:
            hcl.assert_(
                matrix_2[f, 2] == 0, "assert message in the first for loop"
            )  # assert true
            hcl.print(0, "in the first for loop\n")  # should be printed

        with hcl.for_(0, 7, name="for_loop") as f:
            hcl.assert_(
                matrix_2[f, 2] != 0, "assert message in the second for loop"
            )  # assert false
...
def test_for_if():
    ...
    def kernel(matrix_1, matrix_2):
        ...
        matrix_A = hcl.compute(
            (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y] + 7, "matrix_A"
        )
        matrix_B = hcl.compute(
            (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y] + 8, "matrix_B"
        )
        ...
        with hcl.for_(0, 7, name="for_loop") as f:
            with hcl.if_(matrix_1[0, f] == 0):
                ...
                hcl.print(
                    0, "in the first for loop and if statement\n"
                )  # should be printed 7 times

            hcl.print(
                0, "in the first for loop, outside if statement\n"
            )  # should be printed 7 times
        ...
        with hcl.for_(0, 7, name="for_loop") as f:
            with hcl.if_(matrix_1[0, f] == 0):
                ...
                hcl.print(
                    0, "in the second for loop and if statement\n"
                )  # should not be printed

            hcl.print(
                0, "in the second for loop, outside if statement\n"
            )  # should not be printed

        hcl.print(0, "this should not be printed\n")  # should not be printed
        matrix_C = hcl.compute(
            (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y] + 9, "matrix_C"
        )
        matrix_D = hcl.compute(
            (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y] + 10, "matrix_D"
        )
        return return_matrix
...
def test_mem_alloc():
    ...
    def kernel(matrix_1, matrix_2):
        first_matrix = hcl.compute(
            (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y], "first_matrix"
        )
        return_matrix = hcl.compute(
            (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y] + 7, "return_matrix"
        )
