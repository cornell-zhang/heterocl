...
def test_mem_alloc_nested():
    ...
    def kernel(matrix_1, matrix_2):
            ...
            with hcl.for_(0, 2, name="for_loop_in") as h:
                ...
                with hcl.if_(matrix_1[0, 2] >= 0):
                    matrix_C = hcl.compute(
                        (m, k),
                        lambda x, y: matrix_1[x, x] + matrix_2[x, x] + 9,
                        "matrix_C",
                    )
                    hcl.assert_(
                        matrix_1[0, 0] > 0,
                        "assert message in the if statement %d",
                        matrix_C[0, 0],
                    )
                ...
                matrix_F = hcl.compute(
                    (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y] + 8, "matrix_F"
                )
                hcl.print(0, "in for loop\n")

            hcl.assert_(
                matrix_1[0, 0] > 2,
                "assert error, matrix_A[1, 1]: %d matrix_A[2, 1]: %d matrix_A[3, 1]: %d",
                [matrix_A[1, 1], matrix_A[2, 1], matrix_A[3, 1]],
            )
        ...
        matrix_E = hcl.compute(
            (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y] + 10, "matrix_E"
        )
...
def test_mem_if():
    ...
    def kernel(matrix_1, matrix_2):
        ...
        with hcl.if_(matrix_2[0, 0] == 0):
            matrix_A = hcl.compute(
                (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y], "matrix_A"
            )
        with hcl.else_():
            matrix_B = hcl.compute(
                (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y] + 2, "matrix_B"
            )
        ...
        matrix_C = hcl.compute(
            (m, k), lambda x, y: matrix_1[x, y] + matrix_2[x, y], "matrix_C"
        )
