...
def blur_func_gen(target="soda", burst_width=512, unroll_factor=8):
    ...
    hcl_schedule[img_t].stencil(burst_width=burst_width, unroll_factor=unroll_factor)
    hcl_schedule[img_o].stencil(burst_width=burst_width, unroll_factor=unroll_factor)
...
def gaussian_func_gen(input_image, output_image):
    ...
    return hcl.update(
        output_image,
        lambda x, y: hcl.sum(
            input_image[rx + x, ry + y] * kernel(rx) * kernel(ry),
            axis=[rx, ry],
            name="reduce",
            dtype=hcl.Float(),
        ),
...
""",
    ...
    def test_gaussian(self):
        ...
        schedule[gaussian_func_gen.img_o].stencil(burst_width=512, unroll_factor=8)
        self.assertMultiLineEqual(
