  ...
  func @three_mm( %A: tensor<?x?xf32>, %B: tensor<?x?xf32>, %C: tensor<?x?xf32>, %D: tensor<?x?xf32>, %G: tensor<?x?xf32>) -> tensor<?x?xf32>
    {
      ...
      affine.for %x = 0 to 1600 {
        affine.for %y = 0 to 1800 {
          %a1 = constant 0.0 : f32
          %sum = affine.for %r = 0 to 2000 iter_args(%sum_iter = %a1) -> (f32)  {
      ...
      affine.for %x2 = 0 to 1800 {
        affine.for %y1 = 0 to 2200 {
          %a2 = constant 0.0 : f32
          %sum = affine.for %k = 0 to 2400 iter_args(%sum_iter = %a2) -> (f32) {
          ...
          %1 = tensor.insert %sum into %out_CD[%x2, %y1] : tensor<?x?xf32>
        } { loop_name = "y1" }
      ...
      affine.for %x4 = 0 to 1600 {
        affine.for %y2 = 0 to 2200 {
          %a3 = constant 0.0 : f32
          %sum = affine.for %q = 0 to 1800 iter_args(%sum_iter = %a3) -> (f32) {
            ...
            %sum_next = addf %prod3, %sum_iter : f32
            affine.yield %sum_next : f32
          } {loop_name = "q" }
          %2 = tensor.insert %sum into %G[%x4, %y2] : tensor<?x?xf32>
        } { loop_name = "y2" }
