...
def top(target = None):
  ...
  def smith_waterman(ind, seqA, seqB, consensusA, consensusB, matrix):
  ...
  matrix = hcl.placeholder((lenA + 1, lenB + 1), "matrix")

  s = hcl.make_schedule([ind, seqA, seqB, consensusA, consensusB, matrix], smith_waterman)
  print hcl.lower(s, [ind, seqA, seqB, consensusA, consensusB, matrix])
  return hcl.build(s, [ind, seqA, seqB, consensusA, consensusB, matrix], target = target)
...
_matrix = hcl.asarray(np.zeros((lenA + 1, lenB + 1)), hcl.Int())
