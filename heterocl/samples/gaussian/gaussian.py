...
soda_schedule = hcl.make_schedule([input_image, output_image], gaussian)
soda_schedule[gaussian.output].unroll(gaussian.output.axis[1], factor=8)
print(hcl.build(soda_schedule, [input_image, output_image], target='soda'))

llvm_schedule = hcl.make_schedule([input_image, output_image], gaussian)
