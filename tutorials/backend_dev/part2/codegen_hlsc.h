...
using namespace ir;
/*!
 * \brief A class to analyze the IR AST.
 *
 *  Traversing the AST by invoking `AddFunction`. The traverse process
 *  is logged by C-based syntax. The function `Finish` returns the traversed
 *  result in C-based pseudo code.
 *
 *  Note: This class is supposed to be duplicated and modified the developers
...
class CodeGenHLSC :
 ...
 public:
  ...
  std::string Finish();
  /*!
   * \brief Print the Stmt n to CodeGenHLSC->stream
