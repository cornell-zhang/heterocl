...
def test_wrapper(model,ir,frontend,filename,data,tags,_out_shape,input_size,batch_size,target=None):
    ...
    if(ir=="nnvm"):
      f,params = from_nnvm(model,frontend,filename,input_size,batch_size,target=target)
    if(ir=="relay"):
      f,params = get_relay_model(model,(batch_size,*input_size))
    ...
    for i in data:
        ...
        l += 1
        #update progress bar
        printProgressBar(samp_num,tot,prefix="Progress",suffix="Tested",decimals=2,length = 50)
