...
def elemwise_test(in_size,func,dtype):
    ...
    f = hcl.build(s)
    _in1 = (4*(np.random.random(in_size))+1).astype(hcl.dtype_to_str(dtype))
    _in2 = (4*(np.random.random(in_size))+1).astype(hcl.dtype_to_str(dtype))
...
assert_div_float(*elemwise_test((3,3,3,3,3),hlib.elemwise_div,hcl.Float()))
