...
def conv2d_test(in_size,filt_size,stride=[1,1],padding=[0,0],dilation=[1,1],layout="NHWC"):

    I = hcl.placeholder(in_size,"I")
    F = hcl.placeholder(filt_size,  "F")
    def _conv2d(I,F,stride=stride,padding=padding,dilation=dilation,layout=layout):
        return hlib.nn.conv2d(I,F,stride=stride,padding=padding,dilation=dilation,layout=layout)
    ...
    _out = hcl.asarray(np.zeros(output_size(in_size,filt_size,padding,dilation,stride,layout)))
    f(data,filt,_out)
