...
def pool2d(A, kernel_size, stride, padding, pool_mode='max', layout="NHWC"):
    ...
    if layout == "NHWC":
        ...
        output_shape = (A.shape[0],(A.shape[1] - kernel_size[0])//stride[0] + 1,
                    (A.shape[2] - kernel_size[1])//stride[1] + 1,A.shape[3])
        kernel_size = (kernel_size[0], kernel_size[1])
        A_w = as_strided(A, shape = output_shape + kernel_size, 
            strides = (A.strides[0],stride[0]*A.strides[1],
                stride[1]*A.strides[2],A.strides[3]) + (A.strides[1],A.strides[2]))
        ...
        output_shape = (A.shape[0],A.shape[1],(A.shape[2] - kernel_size[0])//stride[0] + 1,
                    (A.shape[3] - kernel_size[1])//stride[1] + 1)
        kernel_size = (kernel_size[0], kernel_size[1])
        A_w = as_strided(A, shape = output_shape + kernel_size, 
            strides = (A.strides[0],A.strides[1],stride[0]*A.strides[2],
...
def pool_test(in_shape,pooling=[2,2],stride=[1,1],padding=[0,0],mode='max',layout="NHWC"):
    ...
    def max_pool(A,pooling=pooling,stride=stride,padding=padding,layout=layout):
        return hlib.nn.max_pool2d(A,pooling,stride,padding,layout)
    def avg_pool(A,pooling=pooling,stride=stride,padding=padding,layout=layout):
        return hlib.nn.avg_pool2d(A,pooling,stride,padding,layout)
    if mode=='max':
