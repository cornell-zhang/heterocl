...
def pool2d(A, kernel_size, stride, padding, pool_mode='max', layout="NHWC"):
    # Padding
    if layout == "NHWC":
        A = np.pad(A, ((0, 0), (padding[0], padding[0]),
                       (padding[1], padding[1]), (0, 0)), mode='constant')
        output_shape = (
            A.shape[0],
        ...
        A = np.pad(
            A, ((0, 0), (0, 0), (padding[0], padding[0]), (padding[1], padding[1])), mode='constant')
        output_shape = (
            A.shape[0],
            A.shape[1],
        ...
        A_w = as_strided(A,
                         ...
                         strides=(A.strides[0],
                                  ...
                                  stride[1] * A.strides[3]) + (A.strides[2],
                                                               A.strides[3]))
...
def pool_test(
    ...
    f(data, out)
    return np.squeeze(
        data.asnumpy()), np.squeeze(
        out.asnumpy()), np.squeeze(real_out)
