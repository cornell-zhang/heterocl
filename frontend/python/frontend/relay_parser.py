...
import hlib
from tvm.relay.expr import Function,Var,Call,Let,If
...
def model_extent(func):
    ...
    if isinstance(func,Call):
        ...
        print(type(func.value),type(func.body))
...
def relay_parser(model,shape,frontend='keras',dtype=hcl.Float()):
    ...
    def parse_rec(node,place):
        if isinstance(node,Function):
            ...
            for i in ty.shape:
                size.append(i.value)
            bind_var=hcl.placeholder(tuple(size),name)
            value = node.value
            val_len = model_extent(value)
            print(type(value))
            temp_var,temp_env=parse_rec(value,place-1)
            if isinstance(value,Var):
                bind_var = fst(temp_var[0])
                ...
                print(args,kwargs)
            env[name] = (name,_convert_map[value.op.name],tuple(args),kwargs)
        ...
        return var,env
    out_var,out_env=parse_rec(body,place_num)
    return out_var,out_env,place_num,params

def get_relay_model(model,shape={},frontend = 'keras',dtype=hcl.Float()):
    out_var,out_env,place_num,params=relay_parser(model,shape,frontend)
    print(out_var)
