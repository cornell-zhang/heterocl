            ...
            if 'attrs' in nodes[ind]:
                ...
                for attr in attrs:
                    if attr in nodes[ind]['attrs']:
                        arg.append(tuple(literal_eval(str(nodes[ind]['attrs'][attr]))))
...
def from_nnvm (model,frontend,filename,input_shape=None,batch_size=None,layout="NCHW",
		dtype=hcl.Float(),target=None):
    if frontend == "keras":
    ...
    for node in _json["arg_nodes"]:
        ...
        if 'input' in node_name:
            gen_placeholders(var_sym,node_name,batch_size,input_shape,False)
        else:
            gen_placeholders(var_sym,node_name,batch_size,params[node_name].shape,True)
    args=[]
    for j in var_sym:
