...
class BasicBlock():
    ...
    def __init__(self, in_planes, planes, stride, params, name="bb"):
        ...
        # create pre-calculated factors
        # params: gamma, beta, moving_mean, moving_var
        # k = (gamma) / sqrt(moving_var + epsilon)
        # h = beta - k * moving_mean
        def pre_calc_bn(gamma, beta, mv, mm):
            base = np.sqrt(mv**2 + 1e-7)
            k = gamma / base
            h = beta - k * mm
            return k, h

        k, h = pre_calc_bn(params[9], params[10], params[11], params[12])
        self.params["bn1"] = [hcl.const_tensor(k, "w_{}_bn1_9".format(name),qtype_float), 
            hcl.const_tensor(h, "w_{}_bn1_10".format(name),qtype_float)] 
        self.params["conv2"] = hcl.const_tensor(params[13],"w_{}_conv2".format(name),hcl.UInt(bitwidth))

        k, h = pre_calc_bn(params[14], params[15], params[16], params[17])
        self.params["bn2"] = [hcl.const_tensor(k, "w_{}_bn2_14".format(name),qtype_float), 
            hcl.const_tensor(h, "w_{}_bn2_15".format(name),qtype_float)]
    ...
    def forward(self, x):
        ...
        bn1, _, _ = nn.batch_norm_pre_calc(conv1, *self.params["bn1"], name=self.name+"_bn1",dtype=qtype_float)
        ...
        bn2, _, _ = nn.batch_norm_pre_calc(conv2, *self.params["bn2"], name=self.name+"_bn2",dtype=qtype_float)
...
class ResNet():
    ...
    def __init__(self, block, num_blocks, params):
        ...
        k, h = pre_calc_bn(params[1], params[2], params[3], params[4])
        self.params["bn1"] = [hcl.const_tensor(k, "w_bn1_0",qtype_float), hcl.const_tensor(h, "w_bn1_1",qtype_float)] 
    ...
    def forward(self, x):
        ...
        bn, _, _ = nn.batch_norm_pre_calc(conv1, *self.params["bn1"], name="bn1",dtype=qtype_float)
