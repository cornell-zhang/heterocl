...
#pragma HLS STREAM variable=PE7_7_fifo2_local depth=2
  ...
  float ret[1000][1][28][28];
  float tanh1[1000][1][28][28];
  for (ap_int<32> args = 0; args < 1000; ++args) {
    for (ap_int<32> args1 = 0; args1 < 28; ++args1) {
      for (ap_int<32> args2 = 0; args2 < 28; ++args2) {
        tanh1[args][0][args1][args2] = ((float)sqrt(((double)ret[args][0][args1][args2])));
      }
    }
  }
  float max_pool2d[1000][1][14][14];
      ...
      for (ap_int<32> w = 0; w < 14; ++w) {
        ...
        for (ap_int<32> ra0 = 0; ra0 < 2; ++ra0) {
          for (ap_int<32> ra1 = 0; ra1 < 2; ++ra1) {
            reducer0 = hls::max(tanh1[i][0][((h * 2) + ra0)][((w * 2) + ra1)], reducer0);
          }
        }
        max_pool2d[i][0][h][w] = reducer0;
      }
    }
  }
  float pad_temp[1000][1][14][14];
  for (ap_int<32> indices = 0; indices < 1000; ++indices) {
    for (ap_int<32> index_tuple = 0; index_tuple < 14; ++index_tuple) {
      for (ap_int<32> i1 = 0; i1 < 14; ++i1) {
        pad_temp[indices][0][index_tuple][i1] = max_pool2d[indices][0][index_tuple][i1];
      }
    }
  }
  float conv2d[1000][50][10][10];
  for (ap_int<32> nn = 0; nn < 1000; ++nn) {
    for (ap_int<32> ff = 0; ff < 50; ++ff) {
      for (ap_int<32> yy = 0; yy < 10; ++yy) {
        for (ap_int<32> xx = 0; xx < 10; ++xx) {
          float sum;
          sum = 0.000000e+00f;
          for (ap_int<32> rc = 0; rc < 20; ++rc) {
            for (ap_int<32> ry = 0; ry < 5; ++ry) {
              for (ap_int<32> rx = 0; rx < 5; ++rx) {
                sum = ((pad_temp[(rc + nn)][0][(yy + ry)][(xx + rx)] * weight_conv2[ff][rc][ry][rx]) + sum);
              }
            }
          }
  ...
  float tanh2[1000][50][10][10];
  for (ap_int<32> args3 = 0; args3 < 1000; ++args3) {
    for (ap_int<32> args0 = 0; args0 < 50; ++args0) {
      for (ap_int<32> args11 = 0; args11 < 10; ++args11) {
        for (ap_int<32> args21 = 0; args21 < 10; ++args21) {
          tanh2[args3][args0][args11][args21] = ((float)sqrt(((double)conv2d[args3][args0][args11][args21])));
        }
      }
    }
  }
  float max_pool2d1[1000][50][5][5];
  for (ap_int<32> i2 = 0; i2 < 1000; ++i2) {
    for (ap_int<32> c = 0; c < 50; ++c) {
      for (ap_int<32> h1 = 0; h1 < 5; ++h1) {
        for (ap_int<32> w1 = 0; w1 < 5; ++w1) {
          float reducer1;
          reducer1 = -1.000000e+00f;
          for (ap_int<32> ra2 = 0; ra2 < 2; ++ra2) {
            for (ap_int<32> ra3 = 0; ra3 < 2; ++ra3) {
              reducer1 = hls::max(tanh2[i2][c][((h1 * 2) + ra2)][((w1 * 2) + ra3)], reducer1);
            }
          }
          max_pool2d1[i2][c][h1][w1] = reducer1;
        }
      }
    }
  }
  float flatten[1000][1250];
  for (ap_int<32> i3 = 0; i3 < 1000; ++i3) {
    for (ap_int<32> j = 0; j < 1250; ++j) {
      flatten[i3][j] = max_pool2d1[i3][(j / 25)][((j / 5) % 5)][(j % 5)];
    }
  }
  ...
  for (ap_int<32> i4 = 0; i4 < 1000; ++i4) {
    for (ap_int<32> j1 = 0; j1 < 500; ++j1) {
      ...
      for (ap_int<32> ra4 = 0; ra4 < 1250; ++ra4) {
        reducer2 = ((flatten[i4][ra4] * weight_fc1[((ra4 / 800) + j1)][(ra4 % 800)]) + reducer2);
      }
      dense[i4][j1] = reducer2;
    }
  }
  float tanh3[1000][500];
  for (ap_int<32> args4 = 0; args4 < 1000; ++args4) {
    for (ap_int<32> args01 = 0; args01 < 500; ++args01) {
      tanh3[args4][args01] = ((float)sqrt(((double)dense[args4][args01])));
    }
  }
  ...
  for (ap_int<32> i5 = 0; i5 < 1000; ++i5) {
    for (ap_int<32> j2 = 0; j2 < 10; ++j2) {
      ...
      for (ap_int<32> ra5 = 0; ra5 < 500; ++ra5) {
        reducer3 = ((tanh3[i5][ra5] * weight_fc2[j2][ra5]) + reducer3);
      }
      dense1[i5][j2] = reducer3;
    }
  }
  float compute0[1000];
  for (ap_int<32> i6 = 0; i6 < 1000; ++i6) {
    ...
    for (ap_int<32> ra6 = 0; ra6 < 10; ++ra6) {
      max = ((ap_int<32>)hls::max(dense1[i6][ra6], ((float)max)));
    }
    compute0[i6] = ((float)max);
  }
  float compute1[1000];
  for (ap_int<32> i7 = 0; i7 < 1000; ++i7) {
    ...
    for (ap_int<32> ra7 = 0; ra7 < 10; ++ra7) {
      sum1 = ((ap_int<32>)(sqrt(((double)(dense1[i7][ra7] - compute0[i7]))) + ((double)sum1)));
    }
    compute1[i7] = ((float)sum1);
  }
  float update0;
  for (ap_int<32> i8 = 0; i8 < 1000; ++i8) {
    for (ap_int<32> j3 = 0; j3 < 10; ++j3) {
      lenet[i8][j3] = ((float)(sqrt(((double)(dense1[i8][j3] - compute0[i8]))) / ((double)compute1[i8])));
    }
  }
}
