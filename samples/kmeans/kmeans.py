...
def top(target = None):
  ...
  def kmeans(points, centers):
    with hcl.stage("S") as S:
      with hcl.for_(0, Loop):
        ...
        U = hcl.mut_compute((N,), lambda n: update_sum(n), "U")
        A = hcl.update(centers, lambda k, d: sum0[k, d]/num0[k], "A")

    return S

  o = hcl.make_schedule([points, centers], kmeans)
  ...
  print hcl.lower(o, [points, centers])
  return hcl.build(o, [points, centers], target = target)
