...
def optical_flow(target=target):
    ...
    def kernel(img0, img1, img2, img3, img4):
       ...
       def calc_xy_gradient(input_image, grad_x, grad_y):
           ...
           def update(y, x):
               with hcl.if_(hcl.and_(y>=2, y<height-2, x>=2, x<width-2)):
                   grad_x[y,x] = sum(input_image[y,x+rx-2] * g_w[rx], axis=rx) / 12.0
                   grad_y[y,x] = sum(input_image[y+ry-2,x] * g_w[ry], axis=ry) / 12.0
       ...
       @hcl.def_([size, size, size, (*size,3)])
       ...
       def outer_product(filt_grad, outer):
           ...
           with hcl.for_(0, height, name="y") as y:
             with hcl.for_(0, width, name="x") as x:
               outer[y,x,0] = filt_grad[y,x,0] * filt_grad[y,x,0]
               outer[y,x,1] = filt_grad[y,x,1] * filt_grad[y,x,1]
               outer[y,x,2] = filt_grad[y,x,2] * filt_grad[y,x,2]
               outer[y,x,3] = filt_grad[y,x,0] * filt_grad[y,x,1]
               outer[y,x,4] = filt_grad[y,x,0] * filt_grad[y,x,2]
               outer[y,x,5] = filt_grad[y,x,1] * filt_grad[y,x,2]
       ...
       output      = hcl.compute((*size,2), init, name="output")
    ...
    s = hcl.create_schedule([image0, image1, image2, image3, image4], kernel)
    ...
    # s.reuse_at(k_grad_y.grad_z, s[k_grad_y], k_grad_y.axis[0])
    # s.reuse_at(k_tensor_x.tensor_y, s[k_tensor_x], k_tensor_x.axis[1])
    # s.reuse_at(k_tensor_y.outer, s[k_tensor_y], k_tensor_y.axis[0])
...
hcl_output = hcl.asarray(np.zeros((*size,2)), dtype)    
...
# read ref optic flow vector 
ref_flo = "datasets/current/ref.flo"
with open(ref_flo, 'rb') as f:
...
for y in range(height):
    for x in range(width):
        ...
        if out_x**2 + out_y**2 > 25:
            out_x = 1e10
            out_y = 1e10

        ref_x, ref_y = ref[y, x]
        ref_deg = np.math.atan2(-ref_y, -ref_x)  * 180 / np.pi
        out_deg = np.math.atan2(-out_y, -out_x)  * 180 / np.pi

        err_deg = out_deg - ref_deg
        while err_deg < -180: err_deg += 360
        while err_deg > 180: err_deg -= 360
        accum_err += abs(err_deg)
        pixel_num += 1

print("Average error: {} degress ".format(accum_err/pixel_num))
