...
def top(target = None):
    ...
    huffman = hcl.placeholder((M,7), 'huffman')
    heap = hcl.placeholder((K,2), 'heap')
    ...
    with hcl.Stage('S') as S:
        ...
        def get_frequency(i):
            temp_ord = hcl.compute((1,), lambda x: train[i])
        ...
        hcl.mut_compute((K-1,), lambda x: update_huffman(x))

        # create encoding from huffman matrix
        X = hcl.mut_compute((1,), lambda x: generate_encode(x), 'X')

    s = hcl.create_schedule([train, text, huffman, heap, encode])
    print hcl.lower(s)
