 ...
 * Use target::llvm, target::cuda etc functions instead of constructing
 * directly.
 */
struct Target {
  ...
  Target(const std::string& target_name, DLDeviceType device_type,
         int max_num_threads, int thread_warp_size,
...
class BuildConfigNode : public Node {
 public:
  /*!
   * \brief The data alignment to use when constructing buffers. If this is set
   * to -1, then TVM's internal default will be used
   */
  int data_alignment = -1;
  /*!
   * \brief The offset factor to use when constructing buffers. If this is set
   * to 0, then the offset field is not used.
   */
  int offset_factor = 0;

  /*!
   * \brief Splitting factor for loop splitting. If this is set to zero, no
   * splitting will be done. Otherwise, a split will be done with this factor
   * and the inner loop will be unrolled.
   */
  int double_buffer_split_loop = 1;
  /*! \brief Threshold of number of steps in the loop to be automatically
   * unrolled */
  int auto_unroll_max_step = 0;
  /*! \brief The maximum nested level of loops that can be automatically
   * unrolled */
  int auto_unroll_max_depth = 8;
  /*! \brief The maximum extent of loop that will be unrolled */
  int auto_unroll_max_extent = 0;
  /*!
   * \brief Whether to explicitly unroll the loop. If set to false, the unroll
   * hint will be passed to the CodeGen phase. Set to true if CodeGen supports
   * unroll pragma.
   */
  bool unroll_explicit = true;

  /*! \brief Set to true if buffer arguments do not overlap. This enables more
   * optimization. */
 ...
 */
EXPORT Array<LoweredFunc> lower(Schedule sch, const Array<Tensor>& args,
                                const std::string& name,
                                const std::unordered_map<Tensor, Buffer>& binds,
                                const BuildConfig& config);

/*!
 * \brief Build a device and host module for a specific target from an array of
 * lowered functions. \param funcs The functions to be built. \param target The
 * target device to build for. \param target_host The target for building host
 * code. If null, a suitable default will be used. \param config The build
 * configuration. \return The built module.
 */
EXPORT runtime::Module build(const Array<LoweredFunc>& funcs,
                             const Target& target, Target* target_host,
                             const BuildConfig& config);
