...
def batch_norm(
    ...
    out = hcl.compute(data.shape, lambda *x: (data[x] - params[get_axis(2, axis, x)]) /
                    (hcl.sqrt(params[get_axis(3, axis, x)] + epsilon)) * params[get_axis(0, axis, x)]
...
def flatten(data, name="flatten", dtype=None):
    ...
    return hcl.compute(oshape, lambda i,j: data[tuple([i] + unwrap(j,ishape[1:]))],
        name=name, dtype=dtype, attrs=OrderedDict([('app_name',tvm.make.StringImm('flatten'))]))
